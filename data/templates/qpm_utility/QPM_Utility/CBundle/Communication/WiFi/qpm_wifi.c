/**
 * @file qpm_wifi.c
 * @brief Implementation of the qpm_wifi C bundle.
 *
...
static char g_qpmWifiLastError[256] = "";

/**
 * @brief Implements the QpmWifi_SetLastError operation.
 * @param message See the matching header for semantic details.
 */
static void QpmWifi_SetLastError(const char *message)
{
    if (message == NULL)
        message = "Wi-Fi/socket error";
    strncpy(g_qpmWifiLastError, message, sizeof(g_qpmWifiLastError) - 1);
    g_qpmWifiLastError[sizeof(g_qpmWifiLastError) - 1] = '\0';
}

/**
 * @brief Implements the QpmWifi_FillIpv4Address operation.
 ...
 */
static int QpmWifi_FillIpv4Address(const char *host, unsigned short port, struct sockaddr_in *address)
{
    if (address == NULL)
        return -1;
    ...
    {
        QpmWifi_SetLastError("host resolution failed");
        return -2;
    }
...
/**
 * @brief Implements the QpmWifi_InitLibrary operation.
 * @return See the matching header for status code or value semantics.
 */
int QpmWifi_InitLibrary(void)
{
    ...
    {
        QpmWifi_SetLastError("WSAStartup failed");
        return -1;
    }
#endif
    return 0;
...
/**
 * @brief Implements the QpmWifi_ShutdownLibrary operation.
 */
...
/**
 * @brief Implements the QpmWifi_Init operation.
...
void QpmWifi_Init(QpmWifiLink *link)
{
    ...
    link->protocol = QPM_WIFI_PROTOCOL_TCP;
    link->isOpen = 0;
}

/**
 * @brief Implements the QpmWifi_OpenTcpClient operation.
 * @param link See the matching header for semantic details.
 * @param host See the matching header for semantic details.
 * @param port See the matching header for semantic details.
 * @return See the matching header for status code or value semantics.
...
int QpmWifi_OpenTcpClient(QpmWifiLink *link, const char *host, unsigned short port)
{
    ...
    if (QpmWifi_FillIpv4Address(host, port, &address) != 0)
        return -2;
    QpmWifiNativeHandle s = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (s == INVALID_SOCKET)
    {
        QpmWifi_SetLastError("socket failed");
        return -3;
    }
    if (connect(s, (struct sockaddr *)&address, sizeof(address)) == SOCKET_ERROR)
    {
        ...
        close(s);
#endif
        QpmWifi_SetLastError("connect failed");
        return -4;
    }
    link->handle = s;
    link->protocol = QPM_WIFI_PROTOCOL_TCP;
...
/**
 * @brief Implements the QpmWifi_OpenTcpServer operation.
 * @param link See the matching header for semantic details.
...
int QpmWifi_OpenTcpServer(QpmWifiLink *link, const char *bindAddress, unsigned short port, int backlog)
{
    ...
    QpmWifi_Close(link);
    struct sockaddr_in address;
    if (QpmWifi_FillIpv4Address(bindAddress, port, &address) != 0)
        return -2;
    QpmWifiNativeHandle s = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (s == INVALID_SOCKET)
        return -3;
    ...
    if (bind(s, (struct sockaddr *)&address, sizeof(address)) == SOCKET_ERROR || listen(s, backlog <= 0 ? 1 : backlog) == SOCKET_ERROR)
    {
        ...
        close(s);
#endif
        QpmWifi_SetLastError("bind/listen failed");
        return -4;
    }
    link->handle = s;
    link->protocol = QPM_WIFI_PROTOCOL_TCP;
...
/**
 * @brief Implements the QpmWifi_AcceptClient operation.
...
{
    if (!QpmWifi_IsOpen(server) || client == NULL)
        return -1;
    QpmWifi_Init(client);
    QpmWifiNativeHandle s = accept(server->handle, NULL, NULL);
    if (s == INVALID_SOCKET)
        return -2;
    client->handle = s;
    client->protocol = QPM_WIFI_PROTOCOL_TCP;
    client->isOpen = 1;
    return 0;
}

/**
 * @brief Implements the QpmWifi_OpenUdp operation.
 * @param link See the matching header for semantic details.
 * @param bindAddress See the matching header for semantic details.
 * @param port See the matching header for semantic details.
 * @return See the matching header for status code or value semantics.
...
int QpmWifi_OpenUdp(QpmWifiLink *link, const char *bindAddress, unsigned short port)
{
    if (link == NULL)
        return -1;
    QpmWifi_Close(link);
    QpmWifiNativeHandle s = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (s == INVALID_SOCKET)
        return -2;
    if (port != 0 || (bindAddress != NULL && bindAddress[0] != '\0'))
    {
        struct sockaddr_in address;
        if (QpmWifi_FillIpv4Address(bindAddress, port, &address) != 0 || bind(s, (struct sockaddr *)&address, sizeof(address)) == SOCKET_ERROR)
        {
#ifdef _WIN32
            closesocket(s);
#else
            close(s);
#endif
            QpmWifi_SetLastError("udp bind failed");
            return -3;
        }
    }
    link->handle = s;
    link->protocol = QPM_WIFI_PROTOCOL_UDP;
    link->isOpen = 1;
    return 0;
}

/**
 * @brief Implements the QpmWifi_Send operation.
 * @param link See the matching header for semantic details.
...
{
    if (!QpmWifi_IsOpen(link) || data == NULL)
        return -1;
...
/**
 * @brief Implements the QpmWifi_Receive operation.
 * @param link See the matching header for semantic details.
...
{
    if (!QpmWifi_IsOpen(link) || buffer == NULL || bufferSize == 0)
        return -1;
...
/**
 * @brief Implements the QpmWifi_SendTo operation.
 * @param link See the matching header for semantic details.
 * @param host See the matching header for semantic details.
 * @param port See the matching header for semantic details.
 * @param data See the matching header for semantic details.
...
/**
 * @brief Implements the QpmWifi_ReceiveFrom operation.
 * @param link See the matching header for semantic details.
 * @param buffer See the matching header for semantic details.
 * @param bufferSize See the matching header for semantic details.
 * @param received See the matching header for semantic details.
...
/**
 * @brief Implements the QpmWifi_Close operation.
 * @param link See the matching header for semantic details.
 */
...
/**
 * @brief Implements the QpmWifi_IsOpen operation.
 * @param link See the matching header for semantic details.
 * @return See the matching header for status code or value semantics.
 */
...
/**
 * @brief Implements the QpmWifi_LastError operation.
 * @return See the matching header for status code or value semantics.
 */
const char *QpmWifi_LastError(void)
{
    return g_qpmWifiLastError;
}
