/**
 * @file qpm_socket.c
 * @brief Implementation of the qpm_socket C bundle.
 *
...
static char g_qpmSocketLastError[256] = "";

/**
 * @brief Implements the QpmSocket_SetLastErrorText operation.
 * @param message See the matching header for semantic details.
 */
static void QpmSocket_SetLastErrorText(const char *message)
{
    if (message == NULL)
        message = "socket error";
    strncpy(g_qpmSocketLastError, message, sizeof(g_qpmSocketLastError) - 1);
    g_qpmSocketLastError[sizeof(g_qpmSocketLastError) - 1] = '\0';
}

/**
 * @brief Implements the QpmSocket_FillAddress operation.
 ...
 */
static int QpmSocket_FillAddress(const char *host, unsigned short port, struct sockaddr_in *address)
{
    if (address == NULL)
        return -1;
...
/**
 * @brief Implements the QpmSocket_InitLibrary operation.
 * @return See the matching header for status code or value semantics.
 */
int QpmSocket_InitLibrary(void)
{
    ...
    {
        QpmSocket_SetLastErrorText("WSAStartup failed");
        return -1;
    }
#endif
    return 0;
...
/**
 * @brief Implements the QpmSocket_ShutdownLibrary operation.
 */
...
/**
 * @brief Implements the QpmSocket_Init operation.
...
/**
 * @brief Implements the QpmSocket_TcpConnect operation.
 * @param socketObj See the matching header for semantic details.
 * @param host See the matching header for semantic details.
 * @param port See the matching header for semantic details.
 * @return See the matching header for status code or value semantics.
...
int QpmSocket_TcpConnect(QpmSocket *socketObj, const char *host, unsigned short port)
{
    ...
    if (QpmSocket_FillAddress(host, port, &address) != 0)
        return -2;
    QpmSocketNativeHandle s = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (s == INVALID_SOCKET)
        return -3;
    if (connect(s, (struct sockaddr *)&address, sizeof(address)) == SOCKET_ERROR)
    {
        ...
        close(s);
#endif
        QpmSocket_SetLastErrorText("connect failed");
        return -4;
    }
    socketObj->handle = s;
...
/**
 * @brief Implements the QpmSocket_TcpListen operation.
 * @param socketObj See the matching header for semantic details.
...
int QpmSocket_TcpListen(QpmSocket *socketObj, const char *bindAddress, unsigned short port, int backlog)
{
    ...
    QpmSocket_Close(socketObj);
    struct sockaddr_in address;
    if (QpmSocket_FillAddress(bindAddress, port, &address) != 0)
        return -2;
    QpmSocketNativeHandle s = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (s == INVALID_SOCKET)
        return -3;
    ...
    if (bind(s, (struct sockaddr *)&address, sizeof(address)) == SOCKET_ERROR || listen(s, backlog <= 0 ? 1 : backlog) == SOCKET_ERROR)
    {
        ...
        close(s);
#endif
        QpmSocket_SetLastErrorText("bind/listen failed");
        return -4;
    }
    socketObj->handle = s;
    socketObj->type = SOCK_STREAM;
...
/**
 * @brief Implements the QpmSocket_TcpAccept operation.
...
{
    if (!QpmSocket_IsOpen(server) || client == NULL)
        return -1;
    QpmSocket_Init(client);
    QpmSocketNativeHandle s = accept(server->handle, NULL, NULL);
    if (s == INVALID_SOCKET)
    {
        QpmSocket_SetLastErrorText("accept failed");
        return -2;
    }
    client->handle = s;
...
/**
 * @brief Implements the QpmSocket_UdpOpen operation.
 * @param socketObj See the matching header for semantic details.
 * @param bindAddress See the matching header for semantic details.
 * @param port See the matching header for semantic details.
 * @return See the matching header for status code or value semantics.
...
int QpmSocket_UdpOpen(QpmSocket *socketObj, const char *bindAddress, unsigned short port)
{
    if (socketObj == NULL)
        return -1;
    QpmSocket_Close(socketObj);
    QpmSocketNativeHandle s = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (s == INVALID_SOCKET)
        return -2;
    if (port != 0 || (bindAddress != NULL && bindAddress[0] != '\0'))
    {
        struct sockaddr_in address;
        if (QpmSocket_FillAddress(bindAddress, port, &address) != 0 || bind(s, (struct sockaddr *)&address, sizeof(address)) == SOCKET_ERROR)
        {
#ifdef _WIN32
            closesocket(s);
#else
            close(s);
#endif
            QpmSocket_SetLastErrorText("udp bind failed");
            return -3;
        }
    }
    socketObj->handle = s;
...
/**
 * @brief Implements the QpmSocket_Send operation.
 * @param socketObj See the matching header for semantic details.
...
int QpmSocket_Send(QpmSocket *socketObj, const void *data, size_t size, size_t *sent)
{
    if (sent != NULL)
        *sent = 0;
    if (!QpmSocket_IsOpen(socketObj) || data == NULL)
        return -1;
    int result = send(socketObj->handle, (const char *)data, (int)size, 0);
    if (result == SOCKET_ERROR)
    {
        QpmSocket_SetLastErrorText("send failed");
        return -2;
    }
    if (sent != NULL)
        *sent = (size_t)result;
...
/**
 * @brief Implements the QpmSocket_Recv operation.
 * @param socketObj See the matching header for semantic details.
...
int QpmSocket_Recv(QpmSocket *socketObj, void *buffer, size_t bufferSize, size_t *received)
{
    if (received != NULL)
        *received = 0;
    if (!QpmSocket_IsOpen(socketObj) || buffer == NULL || bufferSize == 0)
        return -1;
    int result = recv(socketObj->handle, (char *)buffer, (int)bufferSize, 0);
    if (result == SOCKET_ERROR)
    {
        QpmSocket_SetLastErrorText("recv failed");
        return -2;
    }
    if (received != NULL)
...
/**
 * @brief Implements the QpmSocket_UdpSendTo operation.
 * @param socketObj See the matching header for semantic details.
 * @param host See the matching header for semantic details.
 * @param port See the matching header for semantic details.
 * @param data See the matching header for semantic details.
...
int QpmSocket_UdpSendTo(QpmSocket *socketObj, const char *host, unsigned short port, const void *data, size_t size, size_t *sent)
{
    ...
    if (result == SOCKET_ERROR)
    {
        QpmSocket_SetLastErrorText("sendto failed");
        return -3;
    }
    if (sent != NULL)
        *sent = (size_t)result;
...
/**
 * @brief Implements the QpmSocket_UdpRecvFrom operation.
 * @param socketObj See the matching header for semantic details.
 * @param buffer See the matching header for semantic details.
 * @param bufferSize See the matching header for semantic details.
 * @param received See the matching header for semantic details.
...
int QpmSocket_UdpRecvFrom(QpmSocket *socketObj, void *buffer, size_t bufferSize, size_t *received, char *remoteHost, size_t remoteHostSize, unsigned short *remotePort)
{
    ...
    if (result == SOCKET_ERROR)
    {
        QpmSocket_SetLastErrorText("recvfrom failed");
        return -2;
    }
    if (received != NULL)
        *received = (size_t)result;
...
/**
 * @brief Implements the QpmSocket_Close operation.
 * @param socketObj See the matching header for semantic details.
 */
...
/**
 * @brief Implements the QpmSocket_IsOpen operation.
 * @param socketObj See the matching header for semantic details.
 * @return See the matching header for status code or value semantics.
 */
...
/**
 * @brief Implements the QpmSocket_LastError operation.
 * @return See the matching header for status code or value semantics.
 */
const char *QpmSocket_LastError(void)
{
    return g_qpmSocketLastError;
}
