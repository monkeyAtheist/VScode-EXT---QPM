...
#define check_negerror(__x, __msg) do { \
    if ((erreur.code = __x) < 0) { \
        ...
        std::cerr << "[" << qpm_utility::now_timestamp() << "] " << erreur.message << std::endl; \
        qpm_utility::append_error_log(erreur.path, erreur.code, erreur.message, __FILE__, __LINE__, __func__); \
        goto err; \
    } \
    else {erreur.code = 0;}\
} while(0)
