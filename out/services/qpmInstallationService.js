...
exports.QpmInstallationService = void 0;
...
function executableNames(baseNames) {
    const result = [];
...
function findExecutable(root, names, recursive = false) {
...
function findExecutableBelow(root, names, maxDepth) {
...
function scanChildBinDirectories(root, maxDepth) {
    const result = [];
...
function toolchainKind(installation) {
    const paths = [installation.cCompilerExe, installation.cppCompilerExe, installation.clangCcExe, installation.root]
        .filter(Boolean)
        .join(' ')
        .toLowerCase();
    if (paths.includes('clang'))
        return 'Clang/LLVM';
    if (paths.includes('msys64'))
        return 'MSYS2/MinGW';
    if (paths.includes('mingw'))
        return 'MinGW/GCC';
    if (paths.includes('tdm-gcc'))
        return 'TDM-GCC';
    if (paths.includes('gcc') || paths.includes('g++'))
        return 'GCC';
    return 'Qt/C++ toolchain';
}
function pathDetail(value, fallback) {
    return value ? path.basename(value) : fallback;
}
class QpmInstallationService {
    output;
    constructor(output) {
        this.output = output;
    }
    ...
    describe(root, source, recursive = false) {
    ...
    describeExplicitCompilerPaths(source = 'configured') {
        const cCompiler = normalizeExecutableSetting(this.configuration.get('cCompilerPath', ''));
        const cppCompiler = normalizeExecutableSetting(this.configuration.get('cppCompilerPath', ''));
        const archiver = normalizeExecutableSetting(this.configuration.get('archiverPath', ''));
        const debuggerPath = normalizeExecutableSetting(this.configuration.get('debuggerPath', ''));
    ...
    getConfiguredInstallations() {
        const roots = this.configuration.get('installations', []);
        const activeRoot = this.configuration.get('activeInstallation', '').trim();
    ...
    getKnownInstallations(workspaceQpmDir) {
    ...
    getActiveInstallation(workspaceQpmDir, scanIfNeeded = true) {
        const explicit = this.describeExplicitCompilerPaths('configured');
        if (explicit && hasCompiler(explicit)) {
            return explicit;
        }
        const selectedRoot = this.configuration.get('activeInstallation', '').trim();
    ...
    async selectInstallation(workspaceQpmDir) {
    ...
    async promptManualExecutablePaths() {
    ...
    async persistToolchain(installation) {
    ...
    async persistToolchainToTarget(installation, target) {
        ...
        const configured = this.configuration.get('installations', []);
...
exports.QpmInstallationService = QpmInstallationService;
function hasCompiler(installation) {
    return Boolean(installation.cCompilerExe || installation.cppCompilerExe || installation.compileExe || installation.clangCcExe);
}
function normalizeExecutableSetting(value) {
...
async function promptPath(title, value) {
...
function toolchainLabelFromRoot(root) {
    const normalized = root.replace(/\\+$/, '');
    const base = path.basename(normalized);
    return base || normalized;
}
function toolchainLabelFromPath(value) {
