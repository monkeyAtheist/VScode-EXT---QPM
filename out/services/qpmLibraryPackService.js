...
function retiredCatalogMarkerParts() {
...
function isRetiredCatalogIdentifier(value) {
    ...
    if (tokenPrefixes.some((prefix) => tokens.some((token) => token === prefix || token.startsWith(prefix))))
        return true;
    return compactSequences.some((sequence) => compact.includes(sequence));
}
function documentContainsRetiredCatalog(raw) {
    if (!raw || typeof raw !== 'object')
        return false;
    if ([raw.id, raw.name].some(isRetiredCatalogIdentifier))
        return true;
    const inspectLibraries = (libraries) => (Array.isArray(libraries) ? libraries : []).some((library) => isRetiredCatalogIdentifier(library?.id) || isRetiredCatalogIdentifier(library?.name));
    if (inspectLibraries(raw.libraries))
        return true;
    return (Array.isArray(raw.environments) ? raw.environments : []).some((environment) => isRetiredCatalogIdentifier(environment?.id)
        || isRetiredCatalogIdentifier(environment?.name)
        || inspectLibraries(environment?.libraries));
}
function purgeRetiredGlobalPackStorage(targetDirectory, output) {
    const stats = { scannedFiles: 0, deletedFiles: 0, updatedFiles: 0, removedEnvironments: 0, removedLibraries: 0 };
    ...
    for (const fileName of fs.readdirSync(targetDirectory).filter((name) => name.toLowerCase().endsWith('.json')).sort()) {
        ...
        let changed = false;
        const cleanLibraries = (libraries) => {
        ...
        if (Array.isArray(raw?.libraries))
            raw.libraries = cleanLibraries(raw.libraries);
        const environmentLibraryCount = Array.isArray(raw?.environments)
            ? raw.environments.reduce((sum, environment) => sum + (Array.isArray(environment?.libraries) ? environment.libraries.length : 0), 0)
            : 0;
