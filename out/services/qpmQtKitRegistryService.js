...
exports.QpmQtKitRegistryService = void 0;
...
class QpmQtKitRegistryService {
    installations;
    output;
    registryPath;
    constructor(context, installations, output) {
    ...
    async manage() {
        while (true) {
            ...
            if (kitAction.id === 'show') {
                ...
                if (name?.trim())
                    this.upsert({ ...kit, name: name.trim(), updatedAt: new Date().toISOString() });
            }
    ...
    async assignToProject(manifestPath) {
        let kits = this.list();
        if (!kits.length)
            kits = await this.detectAndSave();
        ...
        const projectKit = copyAsProjectKit(choice.kit);
        const index = manifest.profiles.kits.findIndex((entry) => entry.id === projectKit.id);
        if (index >= 0)
            manifest.profiles.kits[index] = projectKit;
        else
            manifest.profiles.kits.push(projectKit);
    ...
    async editKit(initial) {
        ...
        while (true) {
            ...
            if (action.id === 'qt') {
                ...
                if (!selected)
                    continue;
                const installation = (0, qpmQtInstallationService_1.describeQtRoot)(selected);
                if (!installation) {
            ...
            if (action.id === 'arch') {
                ...
                if (selected)
                    kit.architecture = selected.value;
                continue;
            }
            if (action.id === 'family') {
                ...
                if (selected)
                    kit.compilerFamily = selected.value;
                continue;
            }
            if (action.id === 'device') {
            ...
            if (action.id === 'debug-type') {
                ...
                if (selected)
                    kit.debuggerType = selected.value;
                continue;
            }
            if (action.id === 'generator') {
                ...
                if (selected.value === 'custom') {
                    ...
                    if (value !== undefined)
                        kit.generator = value.trim();
                }
                else
                    kit.generator = selected.value;
                continue;
            }
            const field = { cpp: 'compilerPath', c: 'cCompilerPath', debugger: 'debuggerPath', environment: 'environmentScript', qmake: 'qmakePath', cmake: 'cmakePath', build: 'buildToolPath' }[action.id];
            if (field) {
                ...
                if (field === 'compilerPath') {
                    ...
                    const probe = (0, qpmQtInstallationService_1.inspectQtCompiler)(selected, kit.compilerFamily ?? 'unknown', expectedArchitecture);
                    kit = {
                        ...kit,
    ...
    upsert(kit) {
        ...
        if (index >= 0)
            document.kits[index] = kit;
        else
            document.kits.push(kit);
        this.write(document);
    }
    remove(id) {
        const document = this.read();
...
exports.QpmQtKitRegistryService = QpmQtKitRegistryService;
async function selectPath(title, folder, current) {
...
function registeredKitFromInstallation(installation, now) {
