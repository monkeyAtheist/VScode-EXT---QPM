...
exports.QpmColorValueService = void 0;
exports.formatColorValueForTest = formatColorValueForTest;
const vscode = __importStar(require("vscode"));
class QpmColorValueService {
    async openColorValuePicker() {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showErrorMessage('Open a text editor before inserting a color value.');
            return;
        }
        ...
        const panel = vscode.window.createWebviewPanel('qpmColorValuePicker', 'QPM: Insert color value', vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
...
exports.QpmColorValueService = QpmColorValueService;
...
function inferInitialFormat(selectedText) {
...
function buildColorValuePickerHtml(initialHex, initialFormat, initialAlpha) {
    ...
    const formats = [
        ...
        { value: 'hex-0x', label: '0xRRGGBB', description: 'Qt/C++ hexadecimal integer notation' },
        ...
        { value: 'c-rgb-list', label: 'r, g, b', description: 'Decimal channel list for Qt/C++ function arguments' },
        { value: 'c-rgba-list', label: 'r, g, b, a', description: 'Decimal channel list with alpha' },
        { value: 'sdl-color', label: 'SDL_Color initializer', description: 'C initializer compatible with SDL_Color channel ordering' },
        { value: 'vba-rgb', label: 'RGB(r, g, b)', description: 'RGB macro/helper style value, useful for Win32/VBA-like APIs' },
        { value: 'qt-qcolor', label: 'Qt QColor(r, g, b, a)', description: 'Qt QColor constructor' }
    ];
...
<head>
  ...
  <style>
    ...
    .brightness-track { position: relative; width: 44px; min-height: 172px; border-radius: 999px; border: 1px solid var(--vscode-panel-border); background: linear-gradient(to top, #000000, var(--qpm-current-color, #FFFFFF)); display: flex; align-items: center; justify-content: center; padding: 8px 0; }
...
<body>
  ...
  <script>
    ...
    function formatValue(fmt, hx, a) {
      ...
      if (fmt === 'sdl-color') return '{ ' + r + ', ' + g + ', ' + b + ', ' + av + ' }';
      if (fmt === 'vba-rgb') return 'RGB(' + r + ', ' + g + ', ' + b + ')';
      if (fmt === 'qt-qcolor') return 'QColor(' + r + ', ' + g + ', ' + b + ', ' + av + ')';
    ...
    function renderAll() {
      ...
      brightnessTrack.style.setProperty('--qpm-current-color', fullBrightnessHex());
...
function formatColorValueForTest(format, hex, alphaValue = 255) {
    ...
    switch (format) {
        ...
        case 'sdl-color': return `{ ${r}, ${g}, ${b}, ${a} }`;
        case 'vba-rgb': return `RGB(${r}, ${g}, ${b})`;
        case 'qt-qcolor': return `QColor(${r}, ${g}, ${b}, ${a})`;
        case 'hex-css':
