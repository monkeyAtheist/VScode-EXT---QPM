...
exports.QpmTemplateService = exports.SNIPPET_CATEGORIES = void 0;
exports.headerGuardForPath = headerGuardForPath;
exports.renderTemplateText = renderTemplateText;
exports.resolveUirTemplateVersion = resolveUirTemplateVersion;
exports.getBuiltInSnippets = getBuiltInSnippets;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const vscode = __importStar(require("vscode"));
exports.SNIPPET_CATEGORIES = [
...
#endif /* {{guard}} */
`;
function buildLuaExecSourceTemplate() {
    ...
    text = text.replace(`        if (QpmLua_AppendCommandPart(&commandLine, &commandSize, &commandCapacity, config->luaExe) != 0 ||
            (config->unbuffered && QpmLua_AppendCommandPart(&commandLine, &commandSize, &commandCapacity, "-u") != 0) ||
            QpmLua_AppendCommandPart(&commandLine, &commandSize, &commandCapacity, config->scriptPath) != 0)`, `        if (QpmLua_AppendCommandPart(&commandLine, &commandSize, &commandCapacity, config->luaExe) != 0 ||
            QpmLua_AppendCommandPart(&commandLine, &commandSize, &commandCapacity, config->scriptPath) != 0)`);
    text = text.replace(`            argv[index++] = (char *)config->luaExe;
            if (config->unbuffered)
            {
                argv[index++] = (char *)"-u";
            }
            argv[index++] = (char *)config->scriptPath;`, `            argv[index++] = (char *)config->luaExe;
            argv[index++] = (char *)config->scriptPath;`);
...
`;
function validateCppClassName(value) {
    const trimmed = value.trim();
    if (!trimmed)
        return 'A C++ class name is required.';
...
function qtQObjectHeader(className) {
    return `#pragma once\n\n#include <QObject>\n\nclass ${className} final : public QObject\n{\n    Q_OBJECT\n\npublic:\n    explicit ${className}(QObject *parent = nullptr);\n\nsignals:\n\npublic slots:\n\nprivate:\n};\n`;
}
function qtQObjectSource(className, headerFile) {
    return `#include "${headerFile}"\n\n${className}::${className}(QObject *parent)\n    : QObject(parent)\n{\n}\n`;
}
function qtWidgetHeader(className, baseClass) {
...
function headerGuardForPath(filePath) {
...
function resolveUirTemplateVersion(preference, installationRoot) {
...
function sanitizeHeaderText(value, fallback) {
...
function splitLongHeaderWord(word, width) {
    const chunks = [];
...
function wrapHeaderDescription(value, width = HEADER_CHANGE_DESCRIPTION_WIDTH) {
...
function renderHeaderChangeRow(date, author, version, description) {
    return `//** ${fitCell(date, 8)} | ${fitCell(author, 10)} | ${fitCell(version, 7, 'center')} | ${description.padEnd(HEADER_CHANGE_DESCRIPTION_WIDTH, ' ')} **`;
}
function renderHeaderChangeLine(date, author, version, description) {
    const wrappedDescription = wrapHeaderDescription(description);
    return wrappedDescription
        .map((part, index) => renderHeaderChangeRow(index === 0 ? date : '', index === 0 ? author : '', index === 0 ? version : '', part))
        .join('\n');
}
function renderHeaderChangeEntry(date, author, version, description) {
    return `${renderHeaderChangeLine(date, author, version, description)}\n${HEADER_CHANGE_SEPARATOR}`;
}
function renderFileDescriptionHeader(values) {
...
function renderCommentSection(title, style) {
...
const SPECIAL_TEXT_COMPACT_FONT = {
...
const SPECIAL_TEXT_SIZE_PRESETS = [
    {
        id: 'micro',
        label: '1 - Micro',
...
function normalizeSpecialTextPattern(value) {
    const cleaned = String(value ?? '').replace(/[\r\n\t]/g, '').trim();
    return cleaned.slice(0, 8) || '//';
}
function getSingleCharacterToken(pattern) {
    return [...pattern][0] ?? '/';
}
function normalizeSpecialTextSize(value) {
...
function getSpecialTextGlyph(character, font) {
    if (font === 'compact') {
...
function renderSpecialTextRow(row, pattern, horizontalScale) {
...
function renderSpecialCharacterText(text, pattern, size, mode) {
...
async function promptInput(title, prompt, value) {
...
`;
function getBuiltInMyUtilModules() {
    return [
        {
...
class QpmTemplateService {
    ...
    async pickNewFileWorkflow(hasUserTemplates) {
        const groups = [
    ...
    async insertSnippet(requestedCategory) {
        ...
        if (!category) {
            const availableCategories = exports.SNIPPET_CATEGORIES.filter((entry) => entry.id !== 'user' || userSnippets.length > 0);
            const selectedCategory = await vscode.window.showQuickPick(availableCategories.map((entry) => ({
                label: entry.label,
        ...
        const categoryMetadata = exports.SNIPPET_CATEGORIES.find((entry) => entry.id === category);
        const builtInItems = category === 'user'
            ? []
            : builtIns
                .filter((snippet) => category === 'all' || snippet.categories.includes(category))
                .map((snippet) => ({
                label: snippet.label,
                description: snippet.description,
                detail: `Built-in · ${snippet.categories.map((id) => exports.SNIPPET_CATEGORIES.find((entry) => entry.id === id)?.label ?? id).join(' / ')}`,
                body: snippet.body
            }));
        const userItems = category === 'all' || category === 'user'
            ...
            : [];
        const items = [...builtInItems, ...userItems];
        if (items.length === 0) {
    ...
    async insertFileDescriptionHeader() {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
    ...
    async insertSpecialCharacterText() {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
        ...
        const sizeChoice = await vscode.window.showQuickPick(SPECIAL_TEXT_SIZE_PRESETS.map((preset) => ({ ...preset, picked: preset.id === 'standard' })), { title: commandTitle, placeHolder: 'Select the generated text size' });
        if (!sizeChoice)
            return;
        const outputMode = await vscode.window.showQuickPick([
            { label: 'Qt/C++ line comments', description: 'Prefix every generated row with //', value: 'line-comment' },
            { label: 'C block comment', description: 'Wrap generated rows between /* and */', value: 'block-comment' },
            { label: 'Raw characters', description: 'Insert only the generated special characters', value: 'raw' }
    ...
    async saveCurrentFileAsTemplate(filePath) {
        const editor = vscode.window.activeTextEditor;
    ...
    async insertTextAtSelections(editor, text) {
    ...
    async generateCSource(projectDirectory) {
    ...
    async generateCMainWithErrorHandling(projectDirectory) {
        ...
        if (errorModule.value) {
            ...
            files.push({ absolutePath: errorSourcePath, contents: toCrlf(renderTemplateText(ERROR_SOURCE_TEMPLATE, errorVariables)) }, { absolutePath: errorHeaderPath, contents: toCrlf(renderTemplateText(ERROR_HEADER_TEMPLATE, errorVariables)) }, { absolutePath: errorIniPath, contents: toCrlf(renderTemplateText(ERROR_INI_TEMPLATE, errorVariables)) });
        }
        return this.writeFiles(files, sourcePath);
    }
    async generateCppSource(projectDirectory) {
        const choice = await vscode.window.showQuickPick([
    ...
    async generateQtMain(projectDirectory) {
        const choice = await vscode.window.showQuickPick([
    ...
    async generateQtResource(projectDirectory) {
...
`;
    ...
    async generateBoostTest(projectDirectory) {
...
`;
    ...
    async generateQtQuickTest(projectDirectory) {
...
`;
    ...
    async generateHeader(projectDirectory) {
    ...
    async generateModulePair(projectDirectory) {
    ...
    async generateCppClass(projectDirectory) {
    ...
    async generateUir(projectDirectory, includeApplicationSource) {
    ...
    async generateDll(projectDirectory) {
    ...
    async generateErrorModule(projectDirectory) {
    ...
    async generateBundledMyUtilModule(projectDirectory) {
    ...
    async applyBundledModuleProjectHints(module) {
        ...
        const current = configuration.get('libraries', []);
    ...
    async generateBuiltInModuleBundle(projectDirectory, relativeFolder, module) {
    ...
    collectBundledMyUtilFiles(bundleRoot, entries, projectDirectory, targetFolder) {
        const files = [];
    ...
    async generateSingleTextFile(projectDirectory, extension, suggestedBaseName, template, title) {
    ...
    async generateUserTemplate(projectDirectory, templates) {
    ...
    async askTargetPath(projectDirectory, extension, suggestedBaseName, title) {
    ...
    async writeFiles(files, primaryPath, uirPath) {
        ...
        if (existing.length > 0) {
            ...
            const action = await vscode.window.showWarningMessage(`${names} already exist. Choose whether to preserve them or overwrite them with the selected Qt/C++ template.`, { modal: true }, 'Keep existing and add references', 'Overwrite generated files');
            if (!action) {
                return undefined;
            }
    ...
    readBundledUirTemplate() {
        const preference = vscode.workspace.getConfiguration('qpm').get('uirTemplateVersion', 'auto');
    ...
    createVariables(filePath, headerPath, uirPath) {
    ...
    loadFileTemplates() {
        return this.readCollection(this.getFileTemplateStorePath());
    }
    saveFileTemplates(items) {
        this.writeCollection(this.getFileTemplateStorePath(), items);
    }
    loadSnippets() {
        return this.readCollection(this.getSnippetStorePath());
    }
    saveSnippets(items) {
        this.writeCollection(this.getSnippetStorePath(), items);
    }
    readCollection(filePath) {
        if (!fs.existsSync(filePath)) {
            return [];
        }
        try {
    ...
    writeCollection(filePath, items) {
        fs.mkdirSync(path.dirname(filePath), { recursive: true });
        fs.writeFileSync(filePath, `${JSON.stringify({ version: 1, items }, null, 2)}\n`, 'utf8');
    }
    async saveTemplateFromPath(filePath, suppliedContents) {
    ...
    async saveTextAsSnippet(defaultLabel, body) {
    ...
    async openJsonStore(filePath, initial) {
        if (!fs.existsSync(filePath)) {
            fs.mkdirSync(path.dirname(filePath), { recursive: true });
...
exports.QpmTemplateService = QpmTemplateService;
function normalizeRelativeTemplateFolder(value) {
