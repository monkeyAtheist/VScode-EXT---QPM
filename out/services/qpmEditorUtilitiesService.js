...
exports.QpmEditorUtilitiesService = void 0;
// @ts-nocheck
const vscode = __importStar(require("vscode"));
class QpmEditorUtilitiesService {
    async openCharacterTable() { await openCharacterTable(undefined); }
    async convertSelectedTextToDecimalValues() { await convertSelectedTextToDecimalValues(); }
    async convertSelectedNumbersToText() { await convertSelectedNumbersToText(); }
    async openNumberConverter() { await openNumberConverter(undefined); }
    async openTruthTableDesigner() { await openTruthTableDesigner(undefined); }
    async openDigitalFilterDesigner() { await openDigitalFilterDesigner(undefined); }
}
exports.QpmEditorUtilitiesService = QpmEditorUtilitiesService;
async function insertTextAtEditorSelections(editor, text) {
...
function stringToNumericValues(value, mode) {
    ...
    if (mode === 'utf16') {
        ...
        for (let index = 0; index < text.length; index += 1)
            values.push(text.charCodeAt(index));
        return values;
    }
    return Array.from(text).map((character) => character.codePointAt(0) ?? 0);
}
function normalizeNumericToken(token, preferredBase = 'auto') {
...
function numericValuesToString(values, mode) {
    if (mode === 'utf8') {
        if (values.some((value) => !Number.isInteger(value) || value < 0 || value > 0xFF))
            return undefined;
    ...
    if (values.some((value) => !Number.isInteger(value) || value < 0 || value > 0x10FFFF))
        return undefined;
    try {
...
function parseNumericValues(text, preferredBase = 'auto') {
...
async function convertSelectedTextToDecimalValues() {
    ...
    const encoding = await vscode.window.showQuickPick([
        { label: 'Unicode code points', description: 'A → 65, é → 233, 😀 → 128512', value: 'codepoint' },
        { label: 'UTF-8 bytes', description: 'é → 195 169', value: 'utf8' },
        { label: 'UTF-16 code units', description: '😀 → 55357 56832', value: 'utf16' }
...
async function convertSelectedNumbersToText() {
    const editor = vscode.window.activeTextEditor;
    if (!editor || editor.selections.length === 0 || editor.selections.every((selection) => selection.isEmpty)) {
    ...
    const mode = await vscode.window.showQuickPick([
        { label: 'Unicode code points', description: '65 66 67 → ABC', value: 'codepoint' },
        { label: 'UTF-8 bytes', description: '195 169 → é', value: 'utf8' },
        { label: 'UTF-16 code units', description: '55357 56832 → 😀', value: 'utf16' }
    ...
    await editor.edit((builder) => {
        for (const replacement of replacements)
            if (replacement)
                builder.replace(replacement.selection, replacement.text);
    });
}
function buildCharacterTableHtml(initialText) {
    const safeInitialText = String(initialText || 'A');
    const safeInitialJson = JSON.stringify(safeInitialText).replace(/<\/script/gi, '<\\/script');
    const toHexLocal = (value, width = 2) => '0x' + Number(value).toString(16).toUpperCase().padStart(width, '0');
    const toBinLocal = (value, width = 8) => '0b' + Number(value).toString(2).padStart(width, '0');
    ...
    const describeCodeLocal = (value) => {
        if (value < 32)
            return controlNames[value] || 'CTRL';
        if (value === 32)
            return 'SPACE';
        if (value === 127)
            return 'DEL';
    ...
    const printableCharLocal = (value) => {
        if (value === 32)
            return '␠';
    ...
    const utf8BytesForCodePointLocal = (codePoint) => Array.from(Buffer.from(String.fromCodePoint(codePoint), 'utf8'));
    ...
    const renderFallbackRow = (row) => '<tr><td>' + escapeHtml(row.char) + '</td><td>' + escapeHtml(row.name) + '</td><td>' + escapeHtml(row.dec) + '</td><td>' + escapeHtml(row.hex) + '</td><td>' + escapeHtml(row.binary) + '</td><td>' + escapeHtml(row.cEscape) + '</td><td>' + escapeHtml(row.utf8) + '</td><td>' + escapeHtml(row.utf16) + '</td></tr>';
...
async function openCharacterTable(context) {
    const editor = vscode.window.activeTextEditor;
    const initialText = editor && !editor.selection.isEmpty ? editor.document.getText(editor.selection) : 'A';
    const panel = vscode.window.createWebviewPanel('qpmCharacterTable', 'QPM: Character table / converter', vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
...
function buildNumberBitConverterHtml(initialText) {
...
async function openNumberConverter(context) {
    const editor = vscode.window.activeTextEditor;
    const initialText = editor && !editor.selection.isEmpty ? editor.document.getText(editor.selection) : '0x2A';
    const panel = vscode.window.createWebviewPanel('qpmNumberBitConverter', 'QPM: Number / bit converter', vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
...
function buildLogicStateDesignerHtml() {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
async function openTruthTableDesigner(context) {
    const editor = vscode.window.activeTextEditor;
    const panel = vscode.window.createWebviewPanel('qpmTruthTableDesigner', 'QPM: Truth table / FSM designer', vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
...
function buildDigitalFilterDesignerHtml(initialText) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
async function openDigitalFilterDesigner(context) {
    const editor = vscode.window.activeTextEditor;
    const initialText = editor && !editor.selection.isEmpty ? editor.document.getText(editor.selection) : '';
    const panel = vscode.window.createWebviewPanel('qpmDigitalFilterDesigner', 'QPM: Digital filter designer', vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
