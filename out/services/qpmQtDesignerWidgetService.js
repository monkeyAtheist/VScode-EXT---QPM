...
exports.QpmQtDesignerWidgetService = void 0;
exports.qpmDesignerPluginLayout = qpmDesignerPluginLayout;
exports.discoverQpmDesignerWidgets = discoverQpmDesignerWidgets;
exports.generateQpmDesignerPluginProject = generateQpmDesignerPluginProject;
...
function qpmDesignerPluginLayout(projectRoot) {
...
function discoverQpmDesignerWidgets(manifestPath) {
...
function generateQpmDesignerPluginProject(projectRoot, config, targetName = `qpm_${safeIdentifier(path.basename(projectRoot)).toLowerCase()}_designerwidgets`) {
    ...
    const sources = [pluginSourcePath, ...widgets.map((widget) => widget.sourcePath).filter((entry) => !!entry)];
    const forms = widgets.map((widget) => widget.formPath).filter((entry) => !!entry);
    const projectFile = [
...
class QpmQtDesignerWidgetService {
    workspaces;
    installations;
    qtProjects;
    output;
    constructor(workspaces, installations, qtProjects, output) {
        this.workspaces = workspaces;
        this.installations = installations;
        this.qtProjects = qtProjects;
        this.output = output;
    }
    async configure(projectRef) {
    ...
    async prepareAllAndOpen(target, projectRef) {
        const ref = this.requireNativeProject(projectRef);
        if (!ref)
            return;
    ...
    async build(projectRef) {
        ...
        let activeInstallation = this.installations.getActive((0, qtProjectManifest_1.getQtInstallationPreference)(manifest));
        if (!activeInstallation)
            activeInstallation = await this.installations.select();
        if (!activeInstallation)
            return undefined;
        ...
        if (!activeInstallation?.designerPath)
            return undefined;
        if (activeInstallation.designerLauncherKind === 'qtcreator') {
            vscode.window.showErrorMessage("QPM Designer custom widgets require the standalone Qt Widgets Designer (designer.exe), because the plugin must match Designer's Qt ABI. Select designer.exe from the Qt kit used by Designer.");
            return undefined;
        }
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Unable to clean the Designer plugin output. Close Qt Widgets Designer before rebuilding the plugin. ${error instanceof Error ? error.message : String(error)}`);
            return undefined;
        }
    ...
    async install(projectRef) {
        ...
        if (!plugin)
            return undefined;
        const manifest = (0, qtProjectManifest_1.readQtProjectManifest)(ref.absolutePath);
        let installation = this.installations.getActive((0, qtProjectManifest_1.getQtInstallationPreference)(manifest));
        if (!installation)
        ...
        const answer = await vscode.window.showWarningMessage(`Install ${path.basename(plugin)} into the Qt Designer installation? QPM normally uses a project-local plugin path, which is safer.`, { modal: true }, 'Install');
        if (answer !== 'Install')
            return undefined;
        try {
    ...
    async openDesigner(target, projectRef) {
        const ref = this.requireNativeProject(projectRef);
        if (!ref)
            return;
        const projectRoot = path.dirname(ref.absolutePath);
        ...
        if (configured && missingClasses.length > 0) {
            const choice = await vscode.window.showInformationMessage(`${missingClasses.length} project widget(s) are not yet included in the Designer plugin: ${missingClasses.map((widget) => widget.className).join(', ')}.`, 'Prepare all & rebuild', 'Keep current plugin');
        ...
        if (!findPluginBinary(layout.designerDirectory)) {
            const choice = await vscode.window.showInformationMessage('The project-local Qt Designer widget plugin has not been built yet.', 'Build plugin', 'Open without custom widgets');
    ...
    async clean(projectRef) {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Unable to clean the Designer plugin output. Close Qt Widgets Designer first. ${error instanceof Error ? error.message : String(error)}`);
            return;
        }
    ...
    requireNativeProject(projectRef) {
...
exports.QpmQtDesignerWidgetService = QpmQtDesignerWidgetService;
function resolveDesignerKit(activeInstallation) {
    const designerPath = activeInstallation.designerPath;
    if (designerPath) {
        const candidateRoot = path.dirname(path.dirname(designerPath));
        const candidate = (0, qpmQtInstallationService_1.describeQtRoot)(candidateRoot);
        if (candidate)
            return candidate;
    }
    return activeInstallation;
}
function resolveBuildTool(installation) {
...
function createBuildEnvironment(installation) {
    let env = { ...process.env };
...
function findPluginBinary(directory) {
...
function relativeIncludePath(projectRoot, headerPath) {
...
function qmakeList(name, values) {
    if (!values.length)
        return `${name} =`;
