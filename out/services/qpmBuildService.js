...
const qpmDeploymentFileSync_1 = require("./qpmDeploymentFileSync");
class QpmBuildService {
    ...
    async build(rebuild = false, projectRef) {
        ...
        for (const item of order) {
            ...
            if ((0, qtProjectManifest_1.isQtProjectManifestPath)(item.absolutePath)) {
                const manifest = (0, qtProjectManifest_1.readQtProjectManifest)(item.absolutePath);
                if (!(0, qtProjectManifest_1.isQtPythonProject)(manifest) && !await this.deployNativeQtAfterSuccessfulBuild(item, manifest)) {
    ...
    async deployNativeQtAfterSuccessfulBuild(ref, manifest) {
        ...
        this.output.appendLine(`  Automatic standalone deployment: ${buildProfile.name} -> ${(0, qtProjectManifest_1.qtDeploymentDirectory)(ref.absolutePath, this.buildMode, manifest)}`);
    ...
    async deployNativeQtTarget(ref, manifest, installation, announce) {
        ...
        const buildTargetHashBeforeDeploy = process.platform === 'win32' ? (0, qpmDeploymentFileSync_1.sha256File)(buildTargetPath) : undefined;
