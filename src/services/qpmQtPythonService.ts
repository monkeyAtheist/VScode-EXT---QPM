...
export type QpmQtPythonState = 'disabled' | 'missing-python' | 'missing-pyside6' | 'ready' | 'error';

export interface QpmQtPythonTools {
  project?: string;
  designer?: string;
  uic?: string;
  rcc?: string;
...
export interface QpmQtPythonStatus {
  projectName: string;
  manifestPath: string;
  state: QpmQtPythonState;
  interpreter?: string;
  ...
  projectFile?: string;
  entryPoint?: string;
  tools: QpmQtPythonTools;
  message: string;
}

export class QpmQtPythonService implements vscode.Disposable {
  private readonly changeEmitter = new vscode.EventEmitter<void>();
  readonly onDidChange = this.changeEmitter.event;
  private readonly launchedApplications = new Map<string, Set<ChildProcess>>();
  private cachedStatus?: QpmQtPythonStatus;
  ...
  get activeManifestPath(): string | undefined {
    ...
    try {
      return isQtPythonProject(readQtProjectManifest(ref.absolutePath)) ? ref.absolutePath : undefined;
    } catch {
      return undefined;
    }
  }

  get status(): QpmQtPythonStatus | undefined {
    return this.cachedStatus;
  }

  async refresh(): Promise<QpmQtPythonStatus | undefined> {
  ...
  inspect(manifestPath: string): QpmQtPythonStatus {
    ...
    const state: QpmQtPythonState = probe.pySideVersion ? 'ready' : 'missing-pyside6';
    return {
  ...
  async bootstrapActiveProject(interactive = true): Promise<boolean> {
    const manifestPath = this.activeManifestPath;
    if (!manifestPath) return false;
    return this.bootstrap(manifestPath, interactive);
  }

  async bootstrap(manifestPath: string, interactive = true): Promise<boolean> {
    const manifest = readQtProjectManifest(manifestPath);
    if (!isQtPythonProject(manifest)) return false;
    const root = path.dirname(manifestPath);
    ...
    if (manifest.python.autoCreateVirtualEnvironment && !manifest.python.interpreter.trim() && (!venvInterpreter || !fs.existsSync(venvInterpreter))) {
      if (!await this.createVirtualEnvironment(manifestPath, interactive)) return false;
      interpreter = this.resolveInterpreter(manifestPath, readQtProjectManifest(manifestPath));
    } else if (!interpreter && manifest.python.autoCreateVirtualEnvironment) {
      if (!await this.createVirtualEnvironment(manifestPath, interactive)) return false;
      interpreter = this.resolveInterpreter(manifestPath, readQtProjectManifest(manifestPath));
    }
    if (!interpreter) {
      if (interactive) vscode.window.showErrorMessage('Qt for Python: no Python interpreter is available. Configure an interpreter or create the project virtual environment.');
      return false;
    }

    const probe = probePython(interpreter, this.createEnvironment(manifestPath, manifest, interpreter));
    if (!probe.pySideVersion) {
      if (manifest.python.autoInstallPySide6) return this.installPySide6(manifestPath);
      if (interactive) {
        const choice = await vscode.window.showWarningMessage(
          `PySide6 is not installed in ${venv && interpreter.startsWith(venv) ? manifest.python.virtualEnvironment : interpreter}.`,
          'Install PySide6', 'Later'
        );
        if (choice === 'Install PySide6') return this.installPySide6(manifestPath);
      }
      await this.refresh();
      return false;
    }
  ...
  async createVirtualEnvironment(manifestPath = this.requireActiveManifest(), interactive = true): Promise<boolean> {
    const manifest = readQtProjectManifest(manifestPath);
    const root = path.dirname(manifestPath);
    ...
    if (existing && fs.existsSync(existing)) {
      ...
      await this.refresh();
      if (interactive) vscode.window.showInformationMessage(`Qt for Python virtual environment already exists: ${venvPath}`);
      return true;
    }
    const basePython = this.resolveBaseInterpreter(manifestPath, manifest);
    if (!basePython) {
      if (interactive) vscode.window.showErrorMessage('Qt for Python: no base Python 3.10+ interpreter was found to create the virtual environment.');
      return false;
    }
    ...
    await this.refresh();
    if (interactive) vscode.window.showInformationMessage(`Created Qt for Python virtual environment: ${manifest.python.virtualEnvironment}`);
    return true;
  }

  async selectInterpreter(manifestPath = this.requireActiveManifest()): Promise<boolean> {
  ...
  async installPySide6(manifestPath = this.requireActiveManifest()): Promise<boolean> {
    let manifest = readQtProjectManifest(manifestPath);
    let interpreter = this.resolveInterpreter(manifestPath, manifest);
    if (!interpreter && manifest.python.autoCreateVirtualEnvironment) {
      if (!await this.createVirtualEnvironment(manifestPath, false)) return false;
      manifest = readQtProjectManifest(manifestPath);
    ...
    await this.refresh();
    if (ok) vscode.window.showInformationMessage(`${packageName} installed for ${manifest.name}.`);
    return ok;
  }

  async build(manifestPath = this.requireActiveManifest(), rebuild = false): Promise<boolean> {
    const manifest = readQtProjectManifest(manifestPath);
    if (!await this.ensureReady(manifestPath)) return false;
    const status = this.inspect(manifestPath);
    ...
    if (rebuild && !await this.clean(manifestPath)) return false;
  ...
  async clean(manifestPath = this.requireActiveManifest()): Promise<boolean> {
    const manifest = readQtProjectManifest(manifestPath);
    ...
    for (const source of [...files.forms, ...files.resources]) {
      ...
      if (fs.existsSync(generated)) {
        try { fs.rmSync(generated, { force: true }); removed += 1; } catch { /* best effort */ }
      }
    }
  ...
  async run(manifestPath = this.requireActiveManifest(), buildFirst?: boolean): Promise<boolean> {
    const manifest = readQtProjectManifest(manifestPath);
    if (!await this.ensureReady(manifestPath)) return false;
    const shouldBuild = buildFirst ?? manifest.python.buildBeforeRun;
    if (shouldBuild && !await this.build(manifestPath, false)) return false;
    const status = this.inspect(manifestPath);
  ...
  async debug(manifestPath = this.requireActiveManifest()): Promise<boolean> {
    const manifest = readQtProjectManifest(manifestPath);
    if (!await this.ensureReady(manifestPath)) return false;
    const status = this.inspect(manifestPath);
    const interpreter = status.interpreter;
    ...
    if (!started) vscode.window.showWarningMessage('Qt for Python debugging could not be started. Install/enable the Microsoft Python Debugger extension (debugpy) if necessary.');
    return started;
  }

  async deploy(manifestPath = this.requireActiveManifest()): Promise<boolean> {
    const manifest = readQtProjectManifest(manifestPath);
  ...
  async deployAndroid(manifestPath = this.requireActiveManifest()): Promise<boolean> {
    const manifest = readQtProjectManifest(manifestPath);
  ...
  async openDesigner(manifestPath = this.requireActiveManifest(), input?: unknown): Promise<boolean> {
    const manifest = readQtProjectManifest(manifestPath);
    if (!await this.ensureReady(manifestPath)) return false;
    const status = this.inspect(manifestPath);
    ...
    const child = spawn(designer, args, {
  ...
  async compileUiFiles(manifestPath = this.requireActiveManifest()): Promise<boolean> {
    const manifest = readQtProjectManifest(manifestPath);
    if (!await this.ensureReady(manifestPath)) return false;
    const status = this.inspect(manifestPath);
    ...
    const files = resolveQtProjectFiles(manifestPath, manifest).forms.filter((entry) => fs.existsSync(entry));
    for (const file of files) {
      const outputPath = path.join(path.dirname(file), `ui_${path.parse(file).name}.py`);
      if (!await runTool(tool, [file, '-o', outputPath], path.dirname(manifestPath), this.createEnvironment(manifestPath, manifest, status.interpreter), this.output, `pyside6-uic ${path.basename(file)}`)) return false;
    }
    vscode.window.showInformationMessage(`Generated ${files.length} Python UI file(s).`);
    return true;
  }

  async compileResourceFiles(manifestPath = this.requireActiveManifest()): Promise<boolean> {
    const manifest = readQtProjectManifest(manifestPath);
    if (!await this.ensureReady(manifestPath)) return false;
    const status = this.inspect(manifestPath);
    ...
    const files = resolveQtProjectFiles(manifestPath, manifest).resources.filter((entry) => fs.existsSync(entry));
    for (const file of files) {
      const outputPath = path.join(path.dirname(file), `rc_${path.parse(file).name}.py`);
      if (!await runTool(tool, [file, '-o', outputPath], path.dirname(manifestPath), this.createEnvironment(manifestPath, manifest, status.interpreter), this.output, `pyside6-rcc ${path.basename(file)}`)) return false;
    }
    vscode.window.showInformationMessage(`Generated ${files.length} Python resource file(s).`);
    return true;
  }

  async configureVsCodeInterpreter(manifestPath = this.requireActiveManifest(), explicitInterpreter?: string): Promise<boolean> {
    const manifest = readQtProjectManifest(manifestPath);
  ...
  async openReport(manifestPath = this.requireActiveManifest()): Promise<void> {
    const status = this.inspect(manifestPath);
    const manifest = readQtProjectManifest(manifestPath);
    const root = path.dirname(manifestPath);
  ...
  async revealEnvironment(manifestPath = this.requireActiveManifest()): Promise<void> {
    const manifest = readQtProjectManifest(manifestPath);
    const root = path.dirname(manifestPath);
    const venv = resolveProjectPath(root, manifest.python.virtualEnvironment);
  ...
  private requireActiveManifest(): string {
    const manifestPath = this.activeManifestPath;
    if (!manifestPath) throw new Error('The active project is not a Qt for Python project.');
    return manifestPath;
  }

  private async ensureReady(manifestPath: string): Promise<boolean> {
    const status = this.inspect(manifestPath);
    if (status.state === 'ready') return true;
    return this.bootstrap(manifestPath, true);
  }

  private resolveInterpreter(manifestPath: string, manifest: QtProjectManifest): string | undefined {
    const root = path.dirname(manifestPath);
    ...
    if (venvPython && isExecutableFile(venvPython)) return venvPython;
    const workspacePython = vscode.workspace.getConfiguration('python', vscode.Uri.file(root)).get<string>('defaultInterpreterPath', '').trim();
    if (workspacePython) {
  ...
  private resolveBaseInterpreter(manifestPath: string, manifest: QtProjectManifest): string | undefined {
    const root = path.dirname(manifestPath);
    const configured = manifest.python.interpreter.trim();
    if (configured) {
      const candidate = resolveProjectPath(root, configured);
      if (isExecutableFile(candidate) && !pathInside(resolveProjectPath(root, manifest.python.virtualEnvironment), candidate)) return candidate;
    }
    const workspacePython = vscode.workspace.getConfiguration('python', vscode.Uri.file(root)).get<string>('defaultInterpreterPath', '').trim();
    if (workspacePython) {
      const candidate = expandWorkspaceVariables(workspacePython, root);
    ...
    for (const candidate of pythonCommandCandidates()) {
      const check = spawnSync(candidate.command, [...candidate.prefix, '-c', 'import sys; print(sys.executable)'], { cwd: root, encoding: 'utf8', windowsHide: true, timeout: 5000 });
      if (check.status === 0) {
        const executable = String(check.stdout || '').trim().split(/\r?\n/).pop()?.trim();
        if (executable && isExecutableFile(executable)) return executable;
      }
    }
    return undefined;
  }

  private resolveTools(manifestPath: string, manifest: QtProjectManifest, interpreter: string): QpmQtPythonTools {
    ...
    const resolve = (override: string, name: string): string | undefined => {
      const root = path.dirname(manifestPath);
      if (override.trim()) {
      ...
      for (const dir of [scripts, path.dirname(interpreter)].filter(Boolean) as string[]) {
  ...
  private createEnvironment(manifestPath: string, manifest: QtProjectManifest, interpreter?: string): NodeJS.ProcessEnv {
    const root = path.dirname(manifestPath);
    const env: NodeJS.ProcessEnv = { ...process.env, ...manifest.python.environment };
    if (interpreter) {
      const scripts = pythonScriptsDirectory(interpreter, env);
      const parts = [scripts, path.dirname(interpreter), env.PATH].filter(Boolean) as string[];
  ...
  private ensurePyProjectFile(manifestPath: string, manifest: QtProjectManifest): void {
    const root = path.dirname(manifestPath);
    const projectFile = resolveProjectPath(root, manifest.python.projectFile);
    if (fs.existsSync(projectFile)) return;
    if (path.basename(projectFile).toLowerCase() !== 'pyproject.toml') return;
    const files = resolveQtProjectFiles(manifestPath, manifest);
  ...
  private trackApplication(manifestPath: string, child: ChildProcess): void {
    const key = path.resolve(manifestPath).toLowerCase();
    const set = this.launchedApplications.get(key) ?? new Set<ChildProcess>();
    set.add(child);
    this.launchedApplications.set(key, set);
    child.once('exit', () => {
      set.delete(child);
      if (set.size === 0) this.launchedApplications.delete(key);
    });
  }

  private async stopLaunchedApplications(manifestPath: string): Promise<void> {
    const key = path.resolve(manifestPath).toLowerCase();
...
function probePython(interpreter: string, env: NodeJS.ProcessEnv): PythonProbe {
  const code = [
    'import sys',
  ...
  const result = spawnSync(interpreter, ['-c', code], { encoding: 'utf8', env, windowsHide: true, timeout: 8000 });
  if (result.error || result.status !== 0) return { ok: false, error: result.error?.message || String(result.stderr || '').trim() || `exit code ${String(result.status)}` };
...
async function runTool(tool: string, args: string[], cwd: string, env: NodeJS.ProcessEnv | undefined, output: vscode.OutputChannel, label: string): Promise<boolean> {
  ...
  return new Promise<boolean>((resolve) => {
    const child = spawn(tool, args, { cwd, env, windowsHide: true, shell: false });
    ...
    child.on('close', (code) => {
      ...
      if (code !== 0) vscode.window.showErrorMessage(`${label} failed. Open the Qt Project Manager output channel for details.`);
      resolve(code === 0);
    });
  });
}
...
function pythonInVirtualEnvironment(venvPath: string): string | undefined {
  return process.platform === 'win32'
    ? path.join(venvPath, 'Scripts', 'python.exe')
    : path.join(venvPath, 'bin', 'python');
}

function virtualEnvironmentForInterpreter(interpreter: string): string | undefined {
  const parent = path.dirname(interpreter);
  const leaf = path.basename(parent).toLowerCase();
  if (leaf === 'scripts' || leaf === 'bin') return path.dirname(parent);
  return undefined;
}

function pythonScriptsDirectory(interpreter: string, env: NodeJS.ProcessEnv): string {
  const venv = virtualEnvironmentForInterpreter(interpreter);
  if (venv) return process.platform === 'win32' ? path.join(venv, 'Scripts') : path.join(venv, 'bin');
  const result = spawnSync(interpreter, ['-c', 'import sysconfig; print(sysconfig.get_path("scripts") or "")'], { encoding: 'utf8', env, windowsHide: true, timeout: 5000 });
  const candidate = String(result.stdout || '').trim().split(/\r?\n/).pop()?.trim();
  return candidate || path.dirname(interpreter);
}

function pythonCommandCandidates(): Array<{ command: string; prefix: string[] }> {
  return process.platform === 'win32'
...
function expandWorkspaceVariables(value: string, root: string): string {
...
function splitArguments(value: string): string[] {
  ...
  while ((match = pattern.exec(value)) !== null) args.push((match[1] ?? match[2] ?? match[3] ?? '').replace(/\\([\\"'])/g, '$1'));
  return args;
}
...
function pathFromCommandInput(input: unknown): string | undefined {
  ...
  if (typeof input === 'object') {
    ...
    if (typeof value.file?.absolutePath === 'string') return value.file.absolutePath;
    if (typeof value.absolutePath === 'string') return value.absolutePath;
