...
export interface QtInstallerGeneratedPaths {
  ...
  qtIfwRoot: string;
  qtIfwConfig: string;
  qtIfwPackages: string;
  qtIfwPackageXml: string;
  qtIfwComponentScript: string;
  innoRoot: string;
  innoScript: string;
  nsisRoot: string;
  nsisScript: string;
...
export interface QtInstallerIdentity extends QtPackageIdentity {
  installerBaseName: string;
}

export function installerGeneratedPaths(manifestPath: string, componentId: string): QtInstallerGeneratedPaths {
...
export function resolveQtInstallerIdentity(manifest: QtProjectManifest, mode: QpmBuildMode): QtInstallerIdentity {
  ...
  const installerBaseName = sanitizePackageName(pattern.replace(/\$\{(productName|version|platform|arch|configuration|target)\}/g, (_, key: string) => values[key] || ''));
  return { ...identity, installerBaseName };
}

export function renderQtIfwConfig(manifest: QtProjectManifest, identity: QtInstallerIdentity, executableName: string, iconBaseName = ''): string {
...
export function renderQtIfwPackageXml(manifest: QtProjectManifest, identity: QtInstallerIdentity, releaseDate: string, hasScript: boolean): string {
...
export function renderQtIfwComponentScript(manifest: QtProjectManifest, executableName: string): string {
  const installer = manifest.packaging.installer;
...
export function renderInnoSetupScript(manifest: QtProjectManifest, identity: QtInstallerIdentity, projectRoot: string, stageDirectory: string, outputDirectory: string, outputBaseName: string): string {
  const installer = manifest.packaging.installer;
  ...
  const icons: string[] = [];
  if (installer.createStartMenuShortcut && targetExe) icons.push(`Name: "{group}\\${innoEscape(identity.productName)}"; Filename: "{app}\\${innoEscape(targetExe)}"`);
  if (installer.createDesktopShortcut && targetExe) icons.push(`Name: "{autodesktop}\\${innoEscape(identity.productName)}"; Filename: "{app}\\${innoEscape(targetExe)}"`);
...
export function renderNsisScript(manifest: QtProjectManifest, identity: QtInstallerIdentity, stageDirectory: string, outputPath: string): string {
  const installer = manifest.packaging.installer;
  ...
  if (installer.createStartMenuShortcut && targetExe) shortcuts.push(`  CreateDirectory "$SMPROGRAMS\\${nsisEscape(identity.productName)}"\n  CreateShortCut "$SMPROGRAMS\\${nsisEscape(identity.productName)}\\${nsisEscape(identity.productName)}.lnk" "$INSTDIR\\${nsisEscape(targetExe)}"`);
  if (installer.createDesktopShortcut && targetExe) shortcuts.push(`  CreateShortCut "$DESKTOP\\${nsisEscape(identity.productName)}.lnk" "$INSTDIR\\${nsisEscape(targetExe)}"`);
...
export function normalizeNsisVersion(value: string): string {
...
function windowsExecutableName(targetName: string, kind: QtProjectManifest['kind']): string {
  return isRunnableKind(kind) ? `${targetName}.exe` : '';
}

function isRunnableKind(kind: QtProjectManifest['kind']): boolean {
  return kind !== 'static-library' && kind !== 'shared-library';
}

function renderInnoLanguage(language: string): string {
  const normalized = language.trim();
  if (!normalized || normalized.toLowerCase() === 'english') return 'Name: "english"; MessagesFile: "compiler:Default.isl"';
  if (normalized.includes(';') || normalized.includes('MessagesFile:')) return normalized;
...
function jsEscape(value: string): string { return String(value ?? '').replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/[\r\n]+/g, ' '); }
function innoEscape(value: string): string { return String(value ?? '').replace(/"/g, '""').replace(/[\r\n]+/g, ' '); }
function nsisEscape(value: string): string { return String(value ?? '').replace(/\$/g, '$$').replace(/"/g, '$\\"').replace(/[\r\n]+/g, ' '); }
