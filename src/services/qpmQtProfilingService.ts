...
export interface QpmProfilingToolStatus {
  qmlProfilerPath?: string;
  perfPath?: string;
  valgrindPath?: string;
  callgrindAnnotatePath?: string;
  kcachegrindPath?: string;
  cppcheckPath?: string;
  heobPath?: string;
  stracePath?: string;
}

export interface QpmProfilingIssue {
  severity: 'info' | 'warning' | 'error';
  message: string;
}

export interface QpmProfilingReport {
  project: string;
  manifestPath: string;
  targetPath: string;
  outputDirectory: string;
  platform: string;
  tools: QpmProfilingToolStatus;
  issues: QpmProfilingIssue[];
  latestOutput?: string;
}

export interface QpmAnalyzerDiagnostic {
  filePath: string;
  line: number;
  column: number;
...
export class QpmQtProfilingService implements vscode.Disposable {
  private readonly diagnostics = vscode.languages.createDiagnosticCollection('qpm-profiling');
  private readonly disposables: vscode.Disposable[] = [this.diagnostics];
  private activeProcesses = new Set<ChildProcess>();
  private latestOutputByManifest = new Map<string, string>();
  ...
  getReport(projectRef?: QpmWorkspaceProjectRef): QpmProfilingReport | undefined {
    const context = this.resolveContext(projectRef, false);
    if (!context) return undefined;
    const tools = this.getToolStatus(context.ref);
    const issues: QpmProfilingIssue[] = [];
    const platform = getActiveQtPlatformProfile(context.manifest);
    if (!fs.existsSync(context.targetPath)) issues.push({ severity: 'info', message: 'The application target has not been built yet.' });
    if (context.manifest.files.qml.length > 0 && !tools.qmlProfilerPath) issues.push({ severity: 'warning', message: 'qmlprofiler was not found in the selected Qt kit or PATH.' });
    if (platform.type === 'linux-local' || platform.type === 'remote-linux' || process.platform !== 'win32') {
      if (!tools.perfPath && !tools.valgrindPath) issues.push({ severity: 'warning', message: 'Neither perf nor Valgrind is available for CPU profiling.' });
      if (!tools.valgrindPath) issues.push({ severity: 'warning', message: 'Valgrind Memcheck is not available for memory diagnostics.' });
    ...
    if (context.manifest.profiling.cppcheck.enabled && !tools.cppcheckPath) issues.push({ severity: 'warning', message: 'Cppcheck is enabled but the executable was not found.' });
    return {
  ...
  getToolStatus(projectRef?: QpmWorkspaceProjectRef): QpmProfilingToolStatus {
    const context = this.resolveContext(projectRef, false);
  ...
  async runQmlProfiler(): Promise<void> {
    const context = await this.prepareContext();
    const tool = this.getToolStatus(context.ref).qmlProfilerPath;
    if (!tool) throw new Error('qmlprofiler was not found. Select a Qt kit containing qmlprofiler or configure qpm.qmlProfilerPath.');
    if (!context.manifest.profiling.qml.enabled) throw new Error('QML profiling is disabled in Qt Project Settings.');
    ...
    if (result.timedOut) throw new Error(`QML profiling timed out after ${context.manifest.profiling.timeoutMs} ms.`);
    if (result.code !== 0) throw new Error(`qmlprofiler exited with code ${String(result.code)}.`);
  ...
  async runCpuProfiler(): Promise<void> {
    const context = await this.prepareContext();
    const tools = this.getToolStatus(context.ref);
    ...
    if (!tool) throw new Error('No CPU profiler is available. Install perf or Valgrind/Callgrind.');
    if (tool === 'perf') {
      if (!tools.perfPath) throw new Error('perf was selected but was not found.');
      ...
      this.appendResult(result);
      if (result.timedOut) throw new Error('perf profiling timed out.');
      if (result.code !== 0) throw new Error(`perf exited with code ${String(result.code)}.`);
      ...
      if (!tools.valgrindPath) throw new Error('Valgrind was selected but was not found.');
      ...
      this.appendResult(result);
      if (result.timedOut) throw new Error('Callgrind profiling timed out.');
      if (result.code !== 0) throw new Error(`Callgrind exited with code ${String(result.code)}.`);
  ...
  async runMemoryProfiler(): Promise<void> {
    const context = await this.prepareContext();
    const tools = this.getToolStatus(context.ref);
    ...
    if (!tool) throw new Error('No memory profiler is available. Install Valgrind on Linux or Heob on Windows.');
    if (tool === 'heob') {
      if (!tools.heobPath) throw new Error('Heob was selected but was not found.');
      ...
      this.appendResult(result);
      if (result.timedOut) throw new Error('Heob profiling timed out.');
      if (result.code !== 0) throw new Error(`Heob exited with code ${String(result.code)}.`);
      ...
      const result = await runProcess(tools.valgrindPath, args, context.root, context.env, context.manifest.profiling.timeoutMs);
      this.appendResult(result);
      if (result.timedOut) throw new Error('Valgrind Memcheck timed out.');
      if (result.code !== 0 && !fs.existsSync(outputFile)) throw new Error(`Valgrind exited with code ${String(result.code)}.`);
  ...
  async runCppcheck(): Promise<void> {
    const context = this.resolveContext(undefined, true)!;
    const tool = this.getToolStatus(context.ref).cppcheckPath;
    if (!tool) throw new Error('Cppcheck was not found. Configure qpm.cppcheckPath or install Cppcheck.');
    const config = context.manifest.profiling.cppcheck;
    const compileDb = path.join(context.root, 'compile_commands.json');
    if (!fs.existsSync(compileDb)) await vscode.commands.executeCommand('qpm.syncCppTools');
    if (!fs.existsSync(compileDb)) throw new Error(`Compilation database not found: ${compileDb}`);
    ...
    const result = await runProcess(tool, args, context.root, context.env, context.manifest.profiling.timeoutMs);
    this.appendResult(result);
    if (!fs.existsSync(outputFile) && result.code !== 0) throw new Error(`Cppcheck exited with code ${String(result.code)}.`);
  ...
  async runSystemTrace(): Promise<void> {
    const context = await this.prepareContext();
    const tools = this.getToolStatus(context.ref);
    ...
    if (!tool || tool === 'none') throw new Error('System tracing is disabled or strace was not found.');
    if (!tools.stracePath) throw new Error('strace was selected but was not found.');
    ...
    this.appendResult(result);
    if (result.timedOut) throw new Error('System trace timed out.');
    if (result.code !== 0 && !fs.existsSync(outputFile)) throw new Error(`strace exited with code ${String(result.code)}.`);
    this.recordOutput(context, outputFile);
    vscode.window.showInformationMessage(`System trace completed for ${context.manifest.name}.`);
  }

  async openLatestOutput(): Promise<void> {
    const context = this.resolveContext(undefined, true)!;
    const target = this.latestOutputByManifest.get(context.ref.absolutePath) ?? newestFile(context.outputDirectory);
    if (!target || !fs.existsSync(target)) throw new Error('No profiling result is available yet.');
  ...
  stopActiveProfilers(showMessage = true): void {
    for (const child of [...this.activeProcesses]) this.stopTracked(child);
    if (showMessage) vscode.window.showInformationMessage('Active QPM profiler processes stopped.');
  }

  private async prepareContext(): Promise<ProjectContext> {
    ...
    if (context.manifest.profiling.buildBeforeRun) {
      ...
      if (!success) throw new Error('Build failed. Profiling was not started.');
      context = this.resolveContext(context.ref, true)!;
    }
    if (!fs.existsSync(context.targetPath)) throw new Error(`Application target not found: ${context.targetPath}`);
    fs.mkdirSync(context.outputDirectory, { recursive: true });
    return context;
  }

  private resolveContext(projectRef?: QpmWorkspaceProjectRef, required = true): ProjectContext | undefined {
    ...
    const env: NodeJS.ProcessEnv = { ...process.env, ...run.environment, ...manifest.profiling.environment };
    const pathEntries = [path.dirname(targetPath), installation?.binDir, installation?.toolchain.binDir, env.PATH].filter((entry): entry is string => Boolean(entry));
  ...
  private outputPath(context: ProjectContext, template: string, defaultExtension: string): string {
    ...
    if (!path.extname(value) && defaultExtension) value += defaultExtension;
    const resolved = path.resolve(context.outputDirectory, value);
    const relative = path.relative(context.outputDirectory, resolved);
    if (relative.startsWith('..') || path.isAbsolute(relative)) throw new Error(`Profiling output must stay inside ${context.outputDirectory}.`);
    fs.mkdirSync(path.dirname(resolved), { recursive: true });
    return resolved;
  }

  private recordOutput(context: ProjectContext, outputPath: string): void {
    this.latestOutputByManifest.set(context.ref.absolutePath, outputPath);
    this.output.appendLine(`[Qt Profiling] Result: ${outputPath}`);
  }

  private spawnTracked(executable: string, args: string[], cwd: string, env: NodeJS.ProcessEnv): ChildProcess {
    const child = spawn(executable, args, { cwd, env, windowsHide: true, shell: false });
    this.activeProcesses.add(child);
    child.stdout?.on('data', (chunk: Buffer | string) => this.output.append(chunk.toString()));
    child.stderr?.on('data', (chunk: Buffer | string) => this.output.append(chunk.toString()));
    child.once('close', () => this.activeProcesses.delete(child));
    child.once('error', () => this.activeProcesses.delete(child));
    return child;
  }

  private stopTracked(child: ChildProcess): void {
    if (!this.activeProcesses.has(child)) return;
    try { child.kill(); } catch {}
    this.activeProcesses.delete(child);
  }

  private showCommand(label: string, executable: string, args: string[], cwd: string): void {
  ...
  private appendResult(result: ProcessResult): void {
    if (result.stdout.trim()) this.output.appendLine(result.stdout.trimEnd());
    if (result.stderr.trim()) this.output.appendLine(result.stderr.trimEnd());
    this.output.appendLine(`[Qt Profiling] Process exited with code ${String(result.code)}${result.timedOut ? ' (timeout)' : ''}.`);
  }

  private publishDiagnostics(items: QpmAnalyzerDiagnostic[]): void {
    this.diagnostics.clear();
    const grouped = new Map<string, vscode.Diagnostic[]>();
    for (const item of items) {
      if (!item.filePath) continue;
      const diagnostic = new vscode.Diagnostic(
        new vscode.Range(Math.max(0, item.line - 1), Math.max(0, item.column - 1), Math.max(0, item.line - 1), Math.max(1, item.column)),
        item.message,
...
export function parseCppcheckXml(xml: string, projectRoot = process.cwd()): QpmAnalyzerDiagnostic[] {
  const diagnostics: QpmAnalyzerDiagnostic[] = [];
...
export function parseValgrindXml(xml: string, projectRoot = process.cwd()): QpmAnalyzerDiagnostic[] {
  const diagnostics: QpmAnalyzerDiagnostic[] = [];
...
export function splitCommandLine(value: string): string[] {
  ...
  const pattern = /"((?:\\.|[^"\\])*)"|'((?:\\.|[^'\\])*)'|([^\s]+)/g;
  let match: RegExpExecArray | null;
  while ((match = pattern.exec(value)) !== null) result.push((match[1] ?? match[2] ?? match[3] ?? '').replace(/\\([\\"'])/g, '$1'));
  return result;
}

function resolveTool(setting: string, names: string[], installation?: QpmQtInstallation, projectConfigured?: string): string | undefined {
  const configured = String(projectConfigured ?? '').trim() || vscode.workspace.getConfiguration('qpm').get<string>(setting, '').trim();
  const candidates: string[] = [];
  if (configured) candidates.push(configured);
  if (installation) {
    for (const directory of [installation.binDir, installation.toolchain.binDir].filter((entry): entry is string => Boolean(entry))) {
      for (const name of names) candidates.push(executablePath(directory, name));
    }
...
function parseXmlAttributes(value: string): Record<string, string> {
  ...
  let match: RegExpExecArray | null;
  while ((match = pattern.exec(value)) !== null) result[match[1]] = decodeXml(match[2]);
  return result;
}
...
function cppcheckSeverity(value: string): QpmAnalyzerDiagnostic['severity'] {
  if (value === 'error') return 'error';
  if (['warning', 'performance', 'portability'].includes(value)) return 'warning';
  return 'information';
}
