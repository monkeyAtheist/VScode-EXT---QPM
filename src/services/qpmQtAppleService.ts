...
export interface QtAppleSimulator {
  udid: string;
  name: string;
  state: string;
  runtime: string;
...
export interface QtAppleEnvironmentReport {
  platform: QtApplePlatform | 'none';
...
export class QpmQtAppleService implements vscode.Disposable {
  private readonly changed = new vscode.EventEmitter<void>();
  readonly onDidChange = this.changed.event;
  private readonly disposables: vscode.Disposable[] = [];
  private latestArtifactValue = '';
  ...
  get latestArtifactPath(): string { return this.latestArtifactValue; }

  detectEnvironment(profile?: QtPlatformProfile): QtAppleEnvironmentReport {
    ...
    if (platform === 'ios-simulator') {
      if (!simulators.length) details.push('No available iOS simulator was detected. Install an iOS simulator runtime in Xcode.');
      if (!active?.appleSimulatorId) details.push('No simulator is selected; QPM will use a booted simulator or ask at run time.');
    }
    if (platform === 'ios-device') {
      if (!active?.appleDevelopmentTeam) details.push('A development team is normally required for signing and deployment to a physical iOS device.');
      if (!active?.appleDeviceId) details.push('No physical iOS device identifier is configured.');
    }
    if (active?.appleNotaryProfile) details.push(`Notarytool keychain profile: ${active.appleNotaryProfile}.`);
    else if (platform === 'macos') details.push('No notarytool keychain profile is configured.');
  ...
  async configureEnvironment(): Promise<void> {
    ...
    if (!isApplePlatform(profile.type)) {
      const selected = await vscode.window.showQuickPick([
        { label: 'macOS desktop', value: 'macos' as QtApplePlatform },
        { label: 'iOS Simulator', value: 'ios-simulator' as QtApplePlatform },
        { label: 'iOS physical device', value: 'ios-device' as QtApplePlatform }
    ...
    while (true) {
      ...
      if (!action || action.id === 'done') break;
      if (action.id === 'developer') {
        ...
        if (selected?.[0]) profile.appleDeveloperDirectory = selected[0].fsPath;
        ...
        if (value !== undefined) profile.appleBundleIdentifier = value.trim();
        ...
        if (value !== undefined) profile.appleDeploymentTarget = value.trim();
        ...
        if (selected?.length) profile.appleArchitectures = selected.map((entry) => entry.value);
        ...
        if (value !== undefined) profile.appleDevelopmentTeam = value.trim();
        ...
        if (selected?.value === '__custom__') {
          ...
          if (value !== undefined) profile.appleCodeSignIdentity = value.trim();
        } else if (selected) profile.appleCodeSignIdentity = selected.value;
        ...
        if (selected?.[0]) profile.appleEntitlementsFile = projectRelativePath(context.root, selected[0].fsPath);
        ...
        if (value !== undefined) profile.appleNotaryProfile = value.trim();
        ...
        if (tool) {
          ...
          if (selected?.[0]) {
            if (tool.id === 'xcodebuild') profile.appleXcodebuildPath = selected[0].fsPath;
            else if (tool.id === 'xcrun') profile.appleXcrunPath = selected[0].fsPath;
            else profile.appleMacDeployQtPath = selected[0].fsPath;
          }
        }
  ...
  listSimulators(): QtAppleSimulator[] {
    const context = this.readManifestContext(false);
    const profile = context?.platform;
  ...
  async selectSimulator(profile?: QtPlatformProfile, manifestPath?: string, manifest?: QtProjectManifest): Promise<QtAppleSimulator | undefined> {
  ...
  async bootSimulator(): Promise<boolean> {
    const context = this.createContext();
    if (context.platform.type !== 'ios-simulator') throw new Error('Select an iOS Simulator platform profile first.');
    const report = this.detectEnvironment(context.platform);
    const xcrun = report.tools.xcrun;
    if (!xcrun) throw new Error('xcrun was not found.');
    ...
    if (open) await this.run(open, ['-a', 'Simulator'], context.root, context.environment, 'Open Simulator application', true);
    this.changed.fire();
    return true;
  }

  async buildActive(): Promise<boolean> {
    const context = this.createContext();
    if (context.platform.type === 'macos') {
      this.begin(`Build macOS target ${context.manifest.name}`);
      const ok = await this.builds.build(false);
      if (ok) this.latestArtifactValue = findLatestBundle(context.root, context.manifest.targetName, context.buildDirectory) || this.latestArtifactValue;
      this.changed.fire();
      return ok;
    }
  ...
  async deployMacApplication(createDmg?: boolean): Promise<boolean> {
    const context = this.createContext();
    if (context.platform.type !== 'macos') throw new Error('Select a macOS platform profile first.');
    const report = this.detectEnvironment(context.platform);
    const macdeployqt = report.tools.macdeployqt;
    if (!macdeployqt) throw new Error('macdeployqt was not found in the selected Qt kit.');
    ...
    if (!appBundle) throw new Error(`Unable to locate ${context.manifest.targetName}.app after the build.`);

    const profile = context.platform;
    const args: string[] = [path.basename(appBundle), '-verbose=2'];
    const qmlDirs = projectQmlDirectories(context.root, context.manifest);
    for (const directory of qmlDirs) args.push(`-qmldir=${directory}`);
    if (profile.appleAppStoreCompliant) args.push('-appstore-compliant');
    if (profile.appleCodeSignIdentity) {
      if (profile.appleHardenedRuntime && profile.appleTimestamp) args.push(`-sign-for-notarization=${profile.appleCodeSignIdentity}`);
      else {
        args.push(`-codesign=${profile.appleCodeSignIdentity}`);
        if (profile.appleHardenedRuntime) args.push('-hardened-runtime');
        if (profile.appleTimestamp) args.push('-timestamp');
      }
    } else args.push('-codesign=-');
  ...
  async signArtifacts(): Promise<boolean> {
    const context = this.createContext();
    const report = this.detectEnvironment(context.platform);
    const codesign = report.tools.codesign;
    if (!codesign) throw new Error('codesign was not found.');
    if (!context.platform.appleCodeSignIdentity) throw new Error('Configure a code-sign identity first.');
    ...
    if (context.platform.appleHardenedRuntime) args.push('--options', 'runtime');
    if (context.platform.appleTimestamp) args.push('--timestamp');
    if (context.platform.appleEntitlementsFile) args.push('--entitlements', resolveProjectPath(context.root, context.platform.appleEntitlementsFile));
    args.push(artifact);
    ...
    if (success) this.latestArtifactValue = artifact;
    return success;
  }
  ...
  async notarizeArtifact(): Promise<boolean> {
    const context = this.createContext();
    if (!context.platform.appleNotaryProfile) throw new Error('Configure a notarytool keychain profile first.');
    const report = this.detectEnvironment(context.platform);
    const xcrun = report.tools.xcrun;
    if (!xcrun) throw new Error('xcrun was not found.');
    ...
    if (context.platform.appleStapleAfterNotarization) return this.stapleArtifact(artifact);
    return true;
  }

  async stapleArtifact(artifactOverride?: string): Promise<boolean> {
    const context = this.createContext();
    const report = this.detectEnvironment(context.platform);
    const xcrun = report.tools.xcrun;
    if (!xcrun) throw new Error('xcrun was not found.');
  ...
  async installAndRunIosSimulator(): Promise<boolean> {
    const context = this.createContext();
    if (context.platform.type !== 'ios-simulator') throw new Error('Select an iOS Simulator platform profile first.');
    const report = this.detectEnvironment(context.platform);
    const xcrun = report.tools.xcrun;
    ...
    const simulator = await this.ensureSimulator(context);
    if (!simulator) return false;
    if (simulator.state.toLowerCase() !== 'booted' && !await this.bootSimulator()) return false;
    let appBundle = findLatestBundle(context.buildDirectory, context.manifest.targetName, context.root);
    if (!appBundle) {
      if (!await this.buildIos(context)) return false;
      appBundle = findLatestBundle(context.buildDirectory, context.manifest.targetName, context.root);
    }
    if (!appBundle) throw new Error(`Unable to locate ${context.manifest.targetName}.app in the iOS build output.`);
    ...
    if (!await this.run(xcrun, ['simctl', 'install', simulator.udid, appBundle], context.root, context.environment, 'Install application in simulator')) return false;
    ...
    if (success) this.latestArtifactValue = appBundle;
    return success;
  }
  ...
  async revealOutput(): Promise<void> {
    ...
    if (fs.existsSync(candidate)) await vscode.commands.executeCommand('revealFileInOS', vscode.Uri.file(candidate));
    else vscode.window.showWarningMessage('No Apple build or package output exists yet.');
  }
  ...
  private readManifestContext(notify = true): { manifestPath: string; manifest: QtProjectManifest; platform: QtPlatformProfile; root: string } | undefined {
    ...
    if (!isApplePlatform(platform.type)) {
      if (notify) vscode.window.showErrorMessage('Select a macOS, iOS Simulator or iOS Device platform profile first.');
      return undefined;
    }
    return { manifestPath: ref.absolutePath, manifest, platform, root: path.dirname(ref.absolutePath) };
  }

  private createContext(): AppleContext {
    const current = this.readManifestContext();
    if (!current) throw new Error('No active Apple Qt project.');
  ...
  private resolveQtInstallation(manifest: QtProjectManifest): QpmQtInstallation | undefined {
    return this.qtInstallations.getActive(getQtInstallationPreference(manifest, getPersistedQtBuildMode(manifest)));
  }

  private appleBuildDirectory(manifestPath: string, manifest: QtProjectManifest, platform: QtPlatformProfile): string {
    const root = path.dirname(manifestPath);
    const build = getActiveQtBuildProfile(manifest, getPersistedQtBuildMode(manifest));
    const variant = isReleaseBuildMode(getPersistedQtBuildMode(manifest)) ? 'release' : 'debug';
    return path.resolve(root, build.outputDirectory, variant, 'apple', platform.type, platform.id);
  }

  private async buildIos(context: AppleContext): Promise<boolean> {
    if (context.platform.type !== 'ios-simulator' && context.platform.type !== 'ios-device') throw new Error('Select an iOS platform profile first.');
    const report = this.detectEnvironment(context.platform);
    if (!report.hostSupported) throw new Error('iOS builds require macOS.');
    const qtCMake = report.tools['qt-cmake'];
    const xcodebuild = report.tools.xcodebuild;
    if (!qtCMake || !xcodebuild) throw new Error('qt-cmake and xcodebuild are required for the iOS workflow.');
    ...
    if (!await this.run(qtCMake, configureArgs, context.root, context.environment, 'Configure Xcode project with qt-cmake')) return false;
    const xcodeProject = findFirstFile(context.buildDirectory, '.xcodeproj');
    if (!xcodeProject) throw new Error('qt-cmake did not generate an Xcode project.');
    ...
    if (success) this.latestArtifactValue = findLatestBundle(context.buildDirectory, context.manifest.targetName, context.root) || context.buildDirectory;
    this.changed.fire();
    return success;
  }

  private listSimulatorsWithTool(xcrun: string, profile?: QtPlatformProfile): QtAppleSimulator[] {
    try {
      const output = execFileSync(xcrun, ['simctl', 'list', 'devices', 'available', '--json'], { encoding: 'utf8', timeout: 10000, env: createAppleEnvironment(profile), windowsHide: true });
  ...
  private async ensureSimulator(context: AppleContext): Promise<QtAppleSimulator | undefined> {
    const simulators = this.detectEnvironment(context.platform).simulators;
  ...
  private async resolveArtifact(context: AppleContext, extensions: string[]): Promise<string | undefined> {
    const candidates = [
      this.latestArtifactValue,
      findLatestArtifact(context.buildDirectory, extensions),
      findLatestArtifact(context.root, extensions)
    ].filter((entry): entry is string => !!entry && fs.existsSync(entry));
  ...
  try {
    ...
    for (const [runtimeKey, devices] of Object.entries(parsed.devices ?? {})) {
      ...
      for (const device of devices ?? []) {
        ...
        if (udid && available) result.push({ udid, name, state, runtime, available });
      }
    }
...
export function generateAppleCMakeProject(context: Pick<AppleContext, 'root' | 'manifest' | 'platform'> & { manifestPath?: string }): string {
...
function appleBundleIdentifier(profile: QtPlatformProfile, manifest: QtProjectManifest): string {
...
function validateBundleIdentifier(value: string): string | undefined {
  const text = value.trim();
  if (!text) return 'A bundle identifier is required.';
  if (!/^[A-Za-z0-9][A-Za-z0-9.-]*[A-Za-z0-9]$/.test(text) || !text.includes('.')) return 'Use a reverse-domain identifier such as com.company.application.';
  return undefined;
}

function resolveDeveloperDirectory(profile?: QtPlatformProfile): string | undefined {
  if (profile?.appleDeveloperDirectory && isDirectory(profile.appleDeveloperDirectory)) return profile.appleDeveloperDirectory;
  if (process.env.DEVELOPER_DIR && isDirectory(process.env.DEVELOPER_DIR)) return process.env.DEVELOPER_DIR;
  const xcodeSelect = firstExisting(['/usr/bin/xcode-select', findOnPath('xcode-select')]);
  if (!xcodeSelect) return undefined;
  const result = safeExec(xcodeSelect, ['-print-path'], process.env).trim();
  return result && isDirectory(result) ? result : undefined;
}

function resolveWithXcrun(profile: QtPlatformProfile | undefined, tool: string): string | undefined {
  const xcrun = firstExisting([profile?.appleXcrunPath, '/usr/bin/xcrun', findOnPath('xcrun')]);
  if (!xcrun || process.platform !== 'darwin') return undefined;
  const result = safeExec(xcrun, ['--find', tool], createAppleEnvironment(profile)).trim();
  return result && isFile(result) ? result : undefined;
}

function createAppleEnvironment(profile?: QtPlatformProfile, qt?: QpmQtInstallation): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = { ...process.env };
  const developerDirectory = resolveDeveloperDirectory(profile);
  if (developerDirectory) env.DEVELOPER_DIR = developerDirectory;
  const paths = [qt?.binDir, path.dirname(profile?.appleXcodebuildPath || ''), path.dirname(profile?.appleXcrunPath || ''), env.PATH].filter(Boolean) as string[];
  env.PATH = paths.join(path.delimiter);
  return env;
}

function listCodeSigningIdentities(security: string, env: NodeJS.ProcessEnv): string[] {
  const output = safeExec(security, ['find-identity', '-v', '-p', 'codesigning'], env);
  return output.split(/\r?\n/).map((line) => line.match(/"([^"]+)"/)?.[1] || '').filter(Boolean);
}

function safeExec(executable: string, args: string[], env: NodeJS.ProcessEnv): string {
  try { return execFileSync(executable, args, { encoding: 'utf8', env, timeout: 10000, windowsHide: true }); } catch { return ''; }
}
...
function resolveExecutable(configured: string, fallback: string): string | undefined {
  const text = configured.trim();
  if (text && isFile(text)) return text;
  return findOnPath(text || fallback);
}

function firstExisting(values: Array<string | undefined>): string | undefined {
  for (const value of values) if (value && isFile(value)) return value;
  return undefined;
}
...
function projectQmlDirectories(root: string, manifest: QtProjectManifest): string[] {
  const files = resolveQtProjectFiles(path.join(root, `${manifest.name}.qtproject.json`), manifest).qml;
  return [...new Set(files.map((file) => path.dirname(file)).filter(isDirectory))];
}

function findLatestBundle(primaryRoot: string, targetName: string, fallbackRoot: string): string | undefined {
...
function findLatestArtifact(root: string, extensions: string[]): string | undefined {
...
function findFirstFile(root: string, suffix: string): string | undefined {
  return findFiles(root, (entry) => entry.toLowerCase().endsWith(suffix.toLowerCase()), 4)[0];
}

function findFiles(root: string, predicate: (entry: string) => boolean, depth: number): string[] {
  ...
  for (const entry of entries) {
    ...
    if (predicate(absolute)) result.push(absolute);
    if (entry.isDirectory() && !entry.name.startsWith('.') && entry.name !== 'node_modules') result.push(...findFiles(absolute, predicate, depth - 1));
  }
  return result;
}

function newestPath(entries: string[]): string | undefined {
...
function cmakePath(value: string): string { return value.replace(/\\/g, '/'); }
function cmakeQuote(value: string): string { return `"${value.replace(/\\/g, '/').replace(/"/g, '\\"')}"`; }
function cmakeIdentifier(value: string): string { return value.replace(/[^A-Za-z0-9_]+/g, '_').replace(/^([0-9])/, '_$1') || 'QtApplication'; }
function quoteForLog(value: string): string { return /[\s"]/u.test(value) ? `"${value.replace(/"/g, '\\"')}"` : value; }
