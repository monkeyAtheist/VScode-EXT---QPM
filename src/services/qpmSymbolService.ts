...
export interface QpmSourceSymbol {
  name: string;
  signature: string;
  filePath: string;
  line: number;
...
export interface QpmCompletionSymbol {
  name: string;
  signature: string;
  description?: string;
...
export class QpmSymbolService {
  private projectCache?: { key: string; symbols: QpmCompletionSymbol[] };
  ...
  async symbolsForFile(filePath: string): Promise<QpmSourceSymbol[]> {
  ...
  completionSymbols(): QpmCompletionSymbol[] {
  ...
  isQpmWorkspaceFile(filePath: string): boolean {
  ...
  private projectCompletionSymbols(): QpmCompletionSymbol[] {
    ...
    const symbols: QpmCompletionSymbol[] = [];
...
export class QpmCompletionProvider implements vscode.CompletionItemProvider {
  constructor(private readonly symbols: QpmSymbolService) {}
...
function collectPackFunctions(value: unknown, result: QpmCompletionSymbol[]): void {
...
function flattenDocumentSymbols(entries: Array<vscode.DocumentSymbol | vscode.SymbolInformation>, fallbackPath: string): QpmSourceSymbol[] {
  const result: QpmSourceSymbol[] = [];
...
export function scanCFunctions(source: string, filePath: string): QpmSourceSymbol[] {
...
function dedupeSourceSymbols(symbols: QpmSourceSymbol[]): QpmSourceSymbol[] {
...
function dedupeCompletionSymbols(symbols: QpmCompletionSymbol[]): QpmCompletionSymbol[] {
  const map = new Map<string, QpmCompletionSymbol>();
