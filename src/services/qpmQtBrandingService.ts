...
import { QtProjectManifest, hasManagedWindowIcon } from '../model/qtProjectManifest';

export interface QtBrandingArtifacts {
  root: string;
  windowIconSource?: string;
  windowIconCopy?: string;
  windowResource?: string;
  windowStartupSource?: string;
}

export interface QtBrandingValidation {
  errors: string[];
  warnings: string[];
}
...
export function brandingArtifactsRoot(manifestPath: string): string {
  return path.join(path.dirname(manifestPath), '.qpm', 'branding', 'generated');
}

export function validateQtBranding(manifestPath: string, manifest: QtProjectManifest): QtBrandingValidation {
  ...
  if (manifest.branding.executableIcon) {
    ...
    if (!fs.existsSync(icon)) errors.push(`Executable icon was not found: ${manifest.branding.executableIcon}`);
  ...
  if ((manifest.branding.executableIcon || (manifest.branding.windowIcon && manifest.branding.autoApplyWindowIcon))) {
    const externalBackends = manifest.profiles.builds.filter((profile) =>
      (profile.system === 'qmake' || profile.system === 'cmake') && (!!profile.projectFile || !profile.generateProjectFiles)
    );
  ...
  if (manifest.branding.windowIcon) {
    ...
    if (!fs.existsSync(icon)) errors.push(`Qt window icon was not found: ${manifest.branding.windowIcon}`);
    ...
    } else if (!hasManagedWindowIcon(manifest) && manifest.branding.autoApplyWindowIcon) {
...
export function writeQtBrandingArtifacts(manifestPath: string, manifest: QtProjectManifest): QtBrandingArtifacts {
  const root = brandingArtifactsRoot(manifestPath);
  const result: QtBrandingArtifacts = { root };
  if (!hasManagedWindowIcon(manifest)) return result;

  const projectRoot = path.dirname(manifestPath);
  const sourceIcon = resolveProjectPath(projectRoot, manifest.branding.windowIcon);
  if (!fs.existsSync(sourceIcon)) throw new Error(`Qt window icon was not found: ${sourceIcon}`);
...
function renderWindowIconResource(iconFileName: string, alias: string): string {
  return `<RCC>\n  <qresource prefix="/qpm/branding">\n    <file alias="${xmlEscape(alias)}">${xmlEscape(iconFileName)}</file>\n  </qresource>\n</RCC>\n`;
}

function renderWindowIconStartupSource(resourceAlias: string): string {
...
function copyIfChanged(source: string, target: string): void {
  if (fs.existsSync(target)) {
    const a = fs.statSync(source);
    const b = fs.statSync(target);
    if (a.size === b.size && fs.readFileSync(source).equals(fs.readFileSync(target))) return;
  }
...
function cppEscape(value: string): string { return value.replace(/\\/g, '\\\\').replace(/"/g, '\\"'); }
