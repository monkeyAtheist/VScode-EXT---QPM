...
export type QpmSdlEnabledMode = 'off' | 'auto' | 'on';
export type QpmSdlRuntimeMode = 'copy-dlls' | 'path-only' | 'static-link';
export type QpmSdlSubsystem = 'console' | 'windows';
export type QpmSdlArchitecture = 'x86' | 'x64' | 'arm64';
export type QpmSdlVersion = 'auto' | 'SDL2' | 'SDL3';
export type QpmSdlResolvedVersion = 'SDL2' | 'SDL3';

export interface QpmSdlInstallation {
  root: string;
  label: string;
  ...
  packages: string[];
  versions: QpmSdlResolvedVersion[];
  architecture?: QpmSdlArchitecture;
  source: 'configured' | 'scan' | 'manual';
}

export interface QpmSdlConfiguration {
  enabled: QpmSdlEnabledMode;
  version: QpmSdlVersion;
  rootPath: string;
  packages: string[];
  runtimeMode: QpmSdlRuntimeMode;
  subsystem: QpmSdlSubsystem;
  copyAllRuntimeDlls: boolean;
}

export interface QpmSdlBuildPlan {
  version: QpmSdlResolvedVersion;
  rootPath: string;
  includeDirectories: string[];
  libraryDirectory?: string;
  binaryDirectory?: string;
...
export class QpmSdlService implements vscode.Disposable {
  ...
  getConfiguration(): QpmSdlConfiguration {
    return getSdlConfigurationFromWorkspace();
  }

  describe(root: string, source: QpmSdlInstallation['source']): QpmSdlInstallation | undefined {
    return describeSdlRoot(root, source);
  }

  getKnownInstallations(): QpmSdlInstallation[] {
    ...
    const result: QpmSdlInstallation[] = [];
  ...
  async selectInstallation(): Promise<QpmSdlInstallation | undefined> {
    const current = this.getConfiguration();
    const installations = this.getKnownInstallations();
    const choices: Array<vscode.QuickPickItem & { installation?: QpmSdlInstallation; manual?: boolean; disable?: boolean }> = installations.map((installation) => ({
    ...
    const runtime = await vscode.window.showQuickPick([
      { label: 'Copy DLLs beside executable', value: 'copy-dlls' as QpmSdlRuntimeMode, description: 'Recommended on Windows. Copies SDL*.dll and optional dependency DLLs after build.' },
      { label: 'Use PATH only', value: 'path-only' as QpmSdlRuntimeMode, description: 'Do not copy DLLs; prepend the SDL bin folder to PATH when running/debugging.' },
      { label: 'Static link', value: 'static-link' as QpmSdlRuntimeMode, description: `Experimental. Uses static ${version} libraries when available.` }
  ...
  async selectVersion(installation: QpmSdlInstallation, preferredVersion: QpmSdlVersion = 'auto', currentPackages?: string[]): Promise<QpmSdlResolvedVersion | undefined> {
  ...
  async selectPackages(installation: QpmSdlInstallation, version: QpmSdlResolvedVersion, currentPackages?: string[]): Promise<string[] | undefined> {
  ...
  async persist(installation: QpmSdlInstallation, version: QpmSdlResolvedVersion, packages: string[], runtimeMode: QpmSdlRuntimeMode = 'copy-dlls'): Promise<void> {
  ...
  private async persistToTarget(installation: QpmSdlInstallation, version: QpmSdlResolvedVersion, packages: string[], runtimeMode: QpmSdlRuntimeMode, target: vscode.ConfigurationTarget): Promise<void> {
...
export function getSdlConfigurationFromWorkspace(): QpmSdlConfiguration {
...
export function createSdlBuildPlan(config: QpmSdlConfiguration, projectDirectory: string, filePaths: string[], targetType: string, preferredArchitecture?: QpmSdlArchitecture): QpmSdlBuildPlan | undefined {
...
function resolveSdlInstallationForBuild(config: QpmSdlConfiguration, filePaths: string[], preferredArchitecture?: QpmSdlArchitecture): QpmSdlInstallation | undefined {
...
export function describeSdlRoot(root: string, source: QpmSdlInstallation['source'], preferredArchitecture?: QpmSdlArchitecture): QpmSdlInstallation | undefined {
...
export function findSdlIncludeDirectories(root: string, preferredArchitecture?: QpmSdlArchitecture): string[] {
...
export function normalizeSdlPackages(packages: string[], version: QpmSdlVersion = 'auto'): string[] {
...
function normalizeSdlPackageId(entry: string, version: QpmSdlVersion): string | undefined {
...
function resolveVersionedSdlPackage(kind: 'image' | 'mixer' | 'ttf' | 'net', version: QpmSdlVersion): string {
  const resolvedVersion = version === 'SDL3' ? 'SDL3' : 'SDL2';
  return `${resolvedVersion}_${kind}`;
}

function inferSdlPackagesFromSources(filePaths: string[], version: QpmSdlResolvedVersion): string[] {
...
function buildSdlPackageDefines(packages: string[], version: QpmSdlResolvedVersion): string[] {
  const result: string[] = [];
...
function buildSdlLinkArgs(installation: QpmSdlInstallation, packages: string[], version: QpmSdlResolvedVersion, runtimeMode: QpmSdlRuntimeMode, subsystem: QpmSdlSubsystem): string[] {
  const args: string[] = [];
...
function resolveSdlLibraryArgument(libraryDirectory: string | undefined, libraryName: string, runtimeMode: QpmSdlRuntimeMode): string {
...
function staticSdlPackagePrivateLibraries(packages: string[], version: QpmSdlResolvedVersion): string[] {
...
function getSdlRuntimeDlls(installation: QpmSdlInstallation, packages: string[], copyAll: boolean): string[] {
...
function tripletsForArchitecture(architecture: QpmSdlArchitecture): string[] {
  const byArchitecture: Record<QpmSdlArchitecture, string[]> = {
...
function orderedSdlTriplets(preferredArchitecture?: QpmSdlArchitecture): string[] {
  const byArchitecture: Record<QpmSdlArchitecture, string[]> = {
    x86: ['i686-w64-mingw32', 'mingw32'],
    x64: ['x86_64-w64-mingw32', 'mingw64', 'ucrt64', 'clang64'],
    arm64: ['aarch64-w64-mingw32', 'arm64']
...
function orderedSdlLibArchNames(preferredArchitecture?: QpmSdlArchitecture): string[] {
  const byArchitecture: Record<QpmSdlArchitecture, string[]> = {
    x86: ['x86', 'Win32', 'win32'],
...
function findSdlLibraryDirectory(root: string, preferredArchitecture?: QpmSdlArchitecture): string | undefined {
  const candidates = [
...
function findSdlBinaryDirectory(root: string, preferredArchitecture?: QpmSdlArchitecture): string | undefined {
  const candidates = [
...
function detectSdlArchitecture(root: string, binaryDirectory?: string, libraryDirectory?: string, versions: QpmSdlResolvedVersion[] = ['SDL2', 'SDL3']): QpmSdlArchitecture | undefined {
...
function inspectPeArchitecture(filePath: string): QpmSdlArchitecture | undefined {
  try {
...
function normalizeEnabled(value: string | undefined): QpmSdlEnabledMode {
  return value === 'on' || value === 'off' || value === 'auto' ? value : 'auto';
}

function normalizeRuntimeMode(value: string | undefined): QpmSdlRuntimeMode {
  return value === 'copy-dlls' || value === 'path-only' || value === 'static-link' ? value : 'copy-dlls';
}

function normalizeSubsystem(value: string | undefined): QpmSdlSubsystem {
  return value === 'console' || value === 'windows' ? value : 'windows';
}

function normalizeSdlVersion(value: string | undefined): QpmSdlVersion {
  return value === 'SDL2' || value === 'SDL3' || value === 'auto' ? value : 'auto';
}

function resolvePackageVersion(version: QpmSdlVersion, packages: string[]): QpmSdlResolvedVersion {
...
function resolveRequestedVersion(preferred: QpmSdlVersion, packages: string[], availableVersions: QpmSdlResolvedVersion[], filePaths?: string[]): QpmSdlResolvedVersion {
...
function packageVersion(packageId: string): QpmSdlResolvedVersion | undefined {
  return SDL_PACKAGES.find((definition) => definition.id === packageId)?.version;
}

function corePackageId(version: QpmSdlResolvedVersion): string {
  return version;
}

function detectSdlVersions(packages: string[]): QpmSdlResolvedVersion[] {
  const versions: QpmSdlResolvedVersion[] = [];
