...
function generateQmakeProject(context: BackendContext): string {
  ...
  const lines = [
    ...
    `QT += ${context.manifest.qt.modules.map((module) => qmakeModuleName(module)).join(' ')}`,
    `TEMPLATE = ${template}`,
...
function generateCMakeProject(context: BackendContext): string {
  ...
  const qtTargets = context.manifest.qt.modules.map((module) => `Qt${major}::${module}`).join(' ');
  const kind = context.manifest.kind;
...
function backendLinkerFlags(context: BackendContext): string[] {
  if (context.installation.compilerFamily !== 'msvc') return [...context.profile.linkerFlags];
