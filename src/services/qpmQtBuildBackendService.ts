...
export class QpmQtBuildBackendService {
  ...
  async build(manifestPath: string, mode: QpmBuildMode, installation: QpmQtInstallation, rebuild: boolean): Promise<QpmQtBackendResult> {
    const context = this.createContext(manifestPath, mode, installation);
    if (context.profile.system === 'direct') throw new Error('The direct backend is handled by the native direct-build engine.');
  ...
  async clean(manifestPath: string, mode: QpmBuildMode, installation: QpmQtInstallation): Promise<boolean> {
    const context = this.createContext(manifestPath, mode, installation);
    if (context.profile.system === 'direct') return false;
    if (!fs.existsSync(context.buildDirectory)) return true;
  ...
  async configureOnly(manifestPath: string, mode: QpmBuildMode, installation: QpmQtInstallation): Promise<QpmQtBackendResult> {
    const context = this.createContext(manifestPath, mode, installation);
    if (context.profile.system === 'direct') throw new Error('Direct Qt projects do not require a separate configure step.');
    fs.mkdirSync(context.buildDirectory, { recursive: true });
    if (context.profile.system === 'qmake') {
  ...
  async openGeneratedProject(manifestPath: string, mode: QpmBuildMode, installation: QpmQtInstallation): Promise<void> {
  ...
  describe(manifestPath: string, mode: QpmBuildMode, installation: QpmQtInstallation): Record<string, unknown> {
    const context = this.createContext(manifestPath, mode, installation);
    return {
  ...
  private async buildQmake(context: BackendContext): Promise<QpmQtBackendResult> {
    const projectFile = this.resolveOrGenerateQmakeProject(context);
    if (!await this.configureQmake(context, projectFile)) return { success: false, backend: 'qmake', targetPath: context.targetPath, buildDirectory: context.buildDirectory, projectFile };
    const tool = this.resolveQmakeBuildTool(context);
  ...
  private async configureQmake(context: BackendContext, projectFile: string): Promise<boolean> {
    const qmake = context.kit.qmakePath || context.installation.qmakePath;
    if (!qmake || (!fs.existsSync(qmake) && path.isAbsolute(qmake))) throw new Error('qmake was not found for the selected kit.');
  ...
  private resolveOrGenerateQmakeProject(context: BackendContext): string {
    if (context.profile.projectFile) {
      const existing = path.resolve(context.root, context.profile.projectFile);
      if (!fs.existsSync(existing)) throw new Error(`qmake project file not found: ${existing}`);
      return existing;
    }
    if (!context.profile.generateProjectFiles) {
      ...
      if (candidates.length === 1) return path.join(context.root, candidates[0]);
  ...
  private resolveQmakeBuildTool(context: BackendContext): string {
    ...
    if (!tool) throw new Error('No qmake-compatible build tool was found. Select mingw32-make, jom or nmake in the kit. Ninja is reserved for the CMake backend.');
    return tool;
  }

  private async buildCMake(context: BackendContext): Promise<QpmQtBackendResult> {
    const projectFile = this.resolveOrGenerateCMakeProject(context);
    if (!await this.configureCMake(context, projectFile)) return { success: false, backend: 'cmake', targetPath: context.targetPath, buildDirectory: context.buildDirectory, projectFile };
    const cmake = this.resolveCMake(context);
    const args = context.profile.cmakeBuildPreset
  ...
  private async configureCMake(context: BackendContext, projectFile: string): Promise<boolean> {
    const cmake = this.resolveCMake(context);
    let args: string[];
    let cwd: string;
  ...
  private resolveOrGenerateCMakeProject(context: BackendContext): string {
    if (context.profile.projectFile) {
      const configured = path.resolve(context.root, context.profile.projectFile);
      if (!fs.existsSync(configured)) throw new Error(`CMake project path not found: ${configured}`);
      const file = fs.statSync(configured).isDirectory() ? path.join(configured, 'CMakeLists.txt') : configured;
      if (!fs.existsSync(file)) throw new Error(`CMake project file not found: ${file}`);
      return file;
    }
    ...
    if (!context.profile.generateProjectFiles && fs.existsSync(existing)) return existing;
    if (!context.profile.generateProjectFiles) throw new Error(`CMakeLists.txt not found: ${existing}`);
  ...
  private resolveCMake(context: BackendContext): string {
    const cmake = context.kit.cmakePath || context.installation.cmakePath || findOnPath('cmake');
    if (!cmake) throw new Error('CMake was not found. Install CMake or select its executable in the Qt kit.');
    return cmake;
  }

  private publishCompileCommands(context: BackendContext): string | undefined {
  ...
  private createContext(manifestPath: string, mode: QpmBuildMode, installation: QpmQtInstallation): BackendContext {
    ...
    const linkageDiagnostic = validateQtLinkageSelection(installation, profile.linkage, profile.system);
...
function createKitEnvironment(kit: QtKitProfile, installation: QpmQtInstallation): NodeJS.ProcessEnv {
  ...
  const dirs = [installation.binDir, installation.toolchain.binDir, kit.compilerPath ? path.dirname(kit.compilerPath) : undefined, kit.buildToolPath ? path.dirname(kit.buildToolPath) : undefined, kit.cmakePath ? path.dirname(kit.cmakePath) : undefined].filter((entry): entry is string => !!entry);
  ...
  if (installation.qmlDir) env.QML2_IMPORT_PATH = installation.qmlDir;
  return env;
}
...
function generateQmakeProject(context: BackendContext): string {
  ...
  const needsWindowsResource = process.platform === 'win32' && context.manifest.kind !== 'static-library' && (includeWindowsMetadata || !!executableIconPath(context.manifest));
  const windowsBuildResource = needsWindowsResource
    ? writeQtWindowsBuildResource(context.manifestPath, context.manifest, includeWindowsMetadata)
    : undefined;
  const brandingArtifacts = writeQtBrandingArtifacts(context.manifestPath, context.manifest);
...
function generateCMakeProject(context: BackendContext): string {
  const files = resolveQtProjectFiles(context.manifestPath, context.manifest);
  ...
  const brandingArtifacts = writeQtBrandingArtifacts(context.manifestPath, context.manifest);
  const sourceEntries = [
    ...files.sources, ...files.headers, ...files.forms, ...files.resources, ...files.qml,
...
function writeCMakePresets(context: BackendContext, directory: string): void {
  const generator = context.kit.generator || defaultGenerator(context.installation);
...
function cmakeCompilerArguments(context: BackendContext, generator: string): string[] {
  if (/visual studio/i.test(generator)) return [];
...
function defaultGenerator(installation: QpmQtInstallation): string {
  if (installation.ninjaPath) return 'Ninja';
  if (installation.compilerFamily === 'msvc') return installation.ninjaPath ? 'Ninja' : 'NMake Makefiles';
  return process.platform === 'win32' ? 'MinGW Makefiles' : 'Unix Makefiles';
}


function backendTargetPath(manifestPath: string, mode: QpmBuildMode, manifest: QtProjectManifest, installation: QpmQtInstallation): string {
  const standard = qtTargetPath(manifestPath, mode, manifest);
...
function backendCompilerFlags(context: BackendContext): string[] {
  if (context.installation.compilerFamily !== 'msvc') return [...context.profile.compilerFlags];
  const translated: string[] = [];
  for (const flag of context.profile.compilerFlags) {
    if (flag.startsWith('/')) translated.push(flag);
    else if (flag === '-O0') translated.push('/Od');
    else if (flag === '-O1') translated.push('/O1');
    else if (flag === '-O2' || flag === '-O3') translated.push('/O2');
    else if (flag === '-g') translated.push('/Zi');
    else if (flag === '-Wall' || flag === '-Wextra') translated.push('/W4');
    else if (flag === '-fno-omit-frame-pointer') translated.push('/Oy-');
    else if (!flag.startsWith('-')) translated.push(flag);
  }
  return [...new Set(translated)];
}

function backendLinkerFlags(context: BackendContext): string[] {
  const staticRuntimeFlags = compilerStaticRuntimeLinkerFlags(context.installation, context.profile.linkage);
  if (context.installation.compilerFamily !== 'msvc') return [...staticRuntimeFlags, ...context.profile.linkerFlags];
  return context.profile.linkerFlags.filter((flag) => flag.startsWith('/') || !flag.startsWith('-'));
}

function cmakeConfiguration(context: BackendContext): string { return context.profile.variant === 'release' ? 'Release' : 'Debug'; }
function backendLabel(value: string): string { return value === 'qmake' ? 'qmake' : 'CMake'; }
function cppStandardQmake(value: string): string { return value === 'c++23' ? 'c++2b' : value; }
function qmakeModuleName(value: string): string { return value.toLowerCase(); }
function qmakeQuote(value: string): string { return `$$quote(${value.replace(/\\/g, '/')})`; }
function qmakeList(name: string, values: string[]): string { return values.length ? `${name} += ${values.map(qmakeQuote).join(' \\\n  ')}` : ''; }
function cmakeQuote(value: string): string { return `"${value.replace(/\\/g, '/').replace(/"/g, '\\"')}"`; }
function cmakeIdentifier(value: string): string { return value.replace(/[^A-Za-z0-9_]+/g, '_').replace(/^\d/, '_$&') || 'QpmProject'; }
function isGuiKind(kind: string): boolean { return kind === 'widgets-application' || kind === 'quick-application' || kind === 'quick-test-application'; }

function buildToolPriority(tool: string, family: 'mingw' | 'msvc'): number {
  const name = path.basename(tool).toLowerCase();
  if (family === 'mingw') {
    if (name.includes('mingw32-make')) return 0;
    if (name.includes('ninja')) return 1;
    return 5;
  }
  if (name.startsWith('jom')) return 0;
  if (name.startsWith('nmake')) return 1;
  if (name.includes('ninja')) return 2;
  return 5;
}

function makeParallelArguments(tool: string, jobs: number): string[] {
  const name = path.basename(tool).toLowerCase();
  if (jobs <= 0 || name.startsWith('nmake')) return [];
  if (name.startsWith('jom')) return ['-j', String(jobs)];
  if (name.includes('ninja')) return ['-j', String(jobs)];
  return [`-j${jobs}`];
}
function validateTarget(targetPath: string, output: vscode.OutputChannel): boolean {
