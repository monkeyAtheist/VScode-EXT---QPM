...
import { QpmBuildMode, QpmProject, QpmProjectFile, QpmRunOptions, QpmWorkspace, QpmWorkspaceProjectRef } from './types';
import { fromQpmPath, normalizeRelativePath, quote, splitQpmLongValue, toQpmPath, toQpmRuntimeStoragePath, unquote } from '../utils/pathUtils';


export interface QpmVersionInfoSettings {
...
export interface QpmSigningSettings {
  enabled: boolean;
  store: string;
  certificate: string;
  timestampUrl: string;
...
export interface QpmWorkspaceBreakpoint {
  filePath: string;
  line: number;
}

export interface QpmWorkspaceBreakpointSyncResult {
  changed: boolean;
  ...
  ignoredBreakpoints: QpmWorkspaceBreakpoint[];
  trackedBreakpoints: QpmWorkspaceBreakpoint[];
}

export interface QpmNativeTargetSettings {
  targetType: string;
  outputPath: string;
  ...
  versionInfo: QpmVersionInfoSettings;
  signing: QpmSigningSettings;
}
...
function isNativeQpmDocument(filePath: string): boolean {
...
function setPossiblyLongRuntimePathValue(section: IniSection, key: string, value: string): void {
  setPossiblyLongStringValue(section, key, toQpmRuntimeStoragePath(value));
}
...
function setPossiblyLongValue(section: IniSection, key: string, value: string): void {
  ...
  const qpmValue = toQpmPath(value);
  const chunks = splitQpmLongValue(qpmValue);
  if (chunks.length === 1) {
...
function setPossiblyLongStringValue(section: IniSection, key: string, value: string): void {
  ...
  const escapedKey = key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  section.deleteMatching(new RegExp(`^\\s*${escapedKey} Line\\d{4}\\s*=`, 'i'));
  const chunks = splitQpmLongValue(value);
  if (chunks.length === 1) {
    section.set(key, quote(chunks[0]));
    return;
  }
...
function getProjectModeSection(document: IniDocument, mode: QpmBuildMode): IniSection {
...
function resolveStoredPath(section: IniSection, key: string): string {
  ...
  return value ? fromQpmPath(value) : '';
}
...
function setOutputPath(section: IniSection, mode: QpmBuildMode, projectPath: string, value: string): void {
...
function resolveProjectFilePath(projectPath: string, section: IniSection): { absolutePath: string; relativePath?: string } {
  ...
  if (absoluteValue) {
    return { absolutePath: fromQpmPath(absoluteValue), relativePath };
  }
...
function modeSuffix(mode: QpmBuildMode): string {
  switch (mode) {
...
function qpmWorkspaceFileType(projectFileType: string, filePath: string): string {
...
function isBreakpointCompatibleProjectFile(file: QpmProjectFile): boolean {
...
function readWorkspaceFilePath(section: IniSection): string | undefined {
  ...
  return value ? fromQpmPath(value) : undefined;
}
...
function ensureWorkspaceFileSection(
  ...
  section.set('Path', quote(toQpmPath(projectFile.absolutePath)));
  section.set('File Type', quote(qpmWorkspaceFileType(projectFile.type, projectFile.absolutePath)));
...
export class QpmParser {
  parseWorkspace(workspacePath: string): QpmWorkspace {
    ...
    const projects: QpmWorkspaceProjectRef[] = [];
    ...
    for (let index = 1; index <= numberOfProjects; index += 1) {
      ...
      if (exists && absolutePath.toLowerCase().endsWith('.qtproject.json')) {
        try {
          const raw = JSON.parse(readText(absolutePath)) as { name?: unknown };
          if (typeof raw.name === 'string' && raw.name.trim()) projectName = raw.name.trim();
        } catch {
    ...
    return {
      ...
      activeProjectIndex,
      projects,
      qpmDir: reconstructValue(header, 'QPM Dir') ? fromQpmPath(reconstructValue(header, 'QPM Dir')!) : undefined
    };
  }

  parseStandaloneProject(projectPath: string): QpmWorkspace {
    return {
      path: projectPath,
  ...
  parseProject(projectPath: string): QpmProject {
  ...
  getNativeTargetSettings(projectPath: string, mode: QpmBuildMode): QpmNativeTargetSettings {
    const document = IniDocument.parse(readText(projectPath));
    const header = document.getSection('Project Header');
    if (!header) {
  ...
  setNativeTargetSettings(projectPath: string, mode: QpmBuildMode, settings: QpmNativeTargetSettings): void {
    const document = IniDocument.parse(readText(projectPath));
    const header = document.getSection('Project Header');
    if (!header) {
      throw new Error('Invalid Qt project: missing [Project Header].');
  ...
  getTargetPath(projectPath: string, mode: QpmBuildMode): string | undefined {
    const document = IniDocument.parse(readText(projectPath));
    ...
    return absolute ? fromQpmPath(absolute) : undefined;
  }
  ...
  setWorkspaceRunOptions(workspacePath: string, projectIndex: number, mode: QpmBuildMode, options: QpmRunOptions): void {
    if (path.extname(workspacePath).toLowerCase() !== '.cws') {
      return;
    }
    const document = IniDocument.parse(readText(workspacePath));
    ...
    for (const section of document.sections) {
      if (/^Default Build Config \d{4} (?:Debug|Release|Debug64|Release64)$/i.test(section.name) || /^DLL Debugging Support \d{4}$/i.test(section.name)) {
        for (const key of ['Working Directory', 'External Process Path']) {
          const value = reconstructValue(section, key);
          if (value && toQpmRuntimeStoragePath(value) !== value) {
  ...
  repairWorkspaceCompatibility(workspacePath: string): { changed: boolean; changes: string[] } {
    ...
    const normalizePathKey = (section: IniSection, key: string): void => {
      ...
      const normalized = toQpmRuntimeStoragePath(current);
  ...
  getProjectBuildActions(projectPath: string, mode: QpmBuildMode): { preBuildActions: string[]; customBuildActions: string[]; postBuildActions: string[]; nativeSectionsPresent: boolean } {
    const document = IniDocument.parse(readText(projectPath));
  ...
  setProjectBuildActions(projectPath: string, mode: QpmBuildMode, actions: { preBuildActions: string[]; customBuildActions: string[]; postBuildActions: string[] }): void {
    const document = IniDocument.parse(readText(projectPath));
    const modeName = modeSuffix(mode);
  ...
  synchronizeWorkspaceBreakpoints(
    ...
    projectIndex: number,
    projectPath: string,
    requestedBreakpoints: QpmWorkspaceBreakpoint[],
    previouslyTrackedBreakpoints: QpmWorkspaceBreakpoint[] = [],
    preserveNativeBreakpoints = false
  ): QpmWorkspaceBreakpointSyncResult {
    if (path.extname(workspacePath).toLowerCase() !== '.cws') {
    ...
    const ignoredBreakpoints: QpmWorkspaceBreakpoint[] = [];
    ...
    const trackedBreakpoints: QpmWorkspaceBreakpoint[] = [];
    let appliedCount = 0;
  ...
  createWorkspace(rootDirectory: string, workspaceName: string, projectPath: string, qpmDir: string | undefined, formatVersion: number): string {
    ...
    const projectEntry = normalizeRelativePath(rootDirectory, projectPath);
    writeText(workspacePath, this.createMinimalWorkspaceText(workspacePath, projectEntry, qpmDir, formatVersion));
    return workspacePath;
  }

  createWorkspaceAndProject(rootDirectory: string, workspaceName: string, projectName: string, targetType: string, qpmDir: string | undefined, formatVersion: number): { workspacePath: string; projectPath: string } {
    fs.mkdirSync(rootDirectory, { recursive: true });
  ...
  createProject(projectDirectory: string, projectName: string, targetType: string, qpmDir: string | undefined, formatVersion: number): string {
  ...
  private createMinimalWorkspaceText(workspacePath: string, projectFileName: string, qpmDir: string | undefined, version: number): string {
  ...
  private createMinimalProjectText(projectPath: string, projectName: string, targetType: string, qpmDir: string | undefined, version: number): string {
    const document = new IniDocument();
