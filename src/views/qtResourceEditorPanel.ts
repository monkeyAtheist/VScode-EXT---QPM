...
export interface QtResourceFileEntry {
  id: string;
  source: string;
  alias: string;
  empty: boolean;
...
export interface QtResourceGroup {
  id: string;
  prefix: string;
  language: string;
  files: QtResourceFileEntry[];
}

export interface QtResourceDocument {
  groups: QtResourceGroup[];
}

export interface QtResourceValidationIssue {
...
export class QtResourceEditorPanel implements vscode.Disposable {
  private readonly panel: vscode.WebviewPanel;
  private readonly disposables: vscode.Disposable[] = [];
  private document: QtResourceDocument;
  private savedSnapshot: string;
  private statusMessage = '';

  constructor(
    private readonly qrcPath: string,
    ...
    this.panel = vscode.window.createWebviewPanel(
      'qpmQtResourceEditor',
      `Qt Resource Editor — ${path.basename(qrcPath)}`,
  ...
  private async handleMessage(raw: unknown): Promise<void> {
    ...
    const message = raw as { type?: string; document?: QtResourceDocument; groupId?: string; source?: string };
    if (message.document) this.document = normalizeResourceDocument(message.document);
  ...
  private async save(): Promise<void> {
    ...
    if (errors.length > 0) {
      const choice = await vscode.window.showWarningMessage(
        `The resource collection contains ${errors.length} error(s). Save anyway?`,
        { modal: true },
        'Save anyway'
      );
    ...
    if (config.get<boolean>('qrcCreateBackups', true) && fs.existsSync(this.qrcPath)) {
      const backupPath = `${this.qrcPath}.qpm-backup`;
      if (!fs.existsSync(backupPath)) fs.copyFileSync(this.qrcPath, backupPath);
    }
  ...
  private async addFiles(groupId: string | undefined): Promise<void> {
  ...
  private async openResourceFile(source: string | undefined): Promise<void> {
    ...
    try {
      ...
      const choice = await vscode.window.showErrorMessage(
        `Unable to open ${path.basename(absolute)} in VS Code: ${detail}`,
      ...
      if (choice === 'Reveal in File Explorer') {
        ...
        if (!opened) vscode.window.showErrorMessage(`The system application could not open: ${absolute}`);
      }
    }
  }
...
export function readQtResourceDocument(qrcPath: string): QtResourceDocument {
  if (!fs.existsSync(qrcPath)) return { groups: [{ id: crypto.randomUUID(), prefix: '/', language: '', files: [] }] };
  const xml = fs.readFileSync(qrcPath, 'utf8');
  const groups: QtResourceGroup[] = [];
  ...
  while ((groupMatch = groupPattern.exec(xml)) !== null) {
    ...
    const files: QtResourceFileEntry[] = [];
...
export function serializeQtResourceDocument(document: QtResourceDocument): string {
  ...
  for (const group of document.groups) {
    ...
    if (group.language.trim()) attributes.push(`lang="${encodeXmlAttribute(group.language.trim())}"`);
    lines.push(`  <qresource ${attributes.join(' ')}>`);
    for (const file of group.files) {
      const fileAttributes: string[] = [];
      if (file.alias.trim()) fileAttributes.push(`alias="${encodeXmlAttribute(normalizeSlash(file.alias.trim()))}"`);
      if (file.empty) fileAttributes.push('empty="true"');
...
export function validateQtResourceDocument(qrcPath: string, document: QtResourceDocument): QtResourceValidationIssue[] {
  const issues: QtResourceValidationIssue[] = [];
  const qrcDirectory = path.dirname(qrcPath);
  const runtimeKeys = new Map<string, QtResourceFileEntry>();
  for (const group of document.groups) {
    ...
    for (const file of group.files) {
      ...
      if (!fs.existsSync(absolute)) issues.push({ severity: 'error', message: `Missing resource file: ${file.source}`, groupId: group.id, fileId: file.id });
      ...
      if (previous) issues.push({ severity: 'error', message: `Duplicate runtime resource path: ${resourceRuntimePath(prefix, resourceName)}`, groupId: group.id, fileId: file.id });
      else runtimeKeys.set(runtimeKey, file);
    }
  }
  return issues;
}

function normalizeResourceDocument(value: QtResourceDocument): QtResourceDocument {
...
function parseXmlAttributes(source: string): Record<string, string> {
  ...
  while ((match = pattern.exec(source)) !== null) result[match[1]] = decodeXml(match[2] ?? match[3] ?? '');
  return result;
}


function resourceRuntimePath(prefix: string, resourceName: string): string {
...
function encodeXmlText(value: string): string { return value.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'); }
function encodeXmlAttribute(value: string): string { return encodeXmlText(value).replace(/"/g, '&quot;').replace(/'/g, '&apos;'); }
