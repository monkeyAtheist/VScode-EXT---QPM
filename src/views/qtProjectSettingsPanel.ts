...
const PATH_BROWSE_FIELDS: Record<string, { kind: BrowseFieldKind; relative?: boolean; filters?: Record<string, string[]> }> = {
...
function normalizeQtLinkageMode(value: unknown): 'dynamic' | 'static-runtime' | 'static-qt' {
...
function programOutputModeOptions(selected: string): string {
  const values = [
...
function normalizeProgramOutputMode(value: unknown): 'integrated-terminal' | 'output-channel' | 'detached' {
...
function pythonUiModeOptions(selected: string): string { return [['compiled','Compile .ui to ui_*.py'],['runtime','Runtime .ui loading / manual workflow']].map(([value,label]) => `<option value="${value}" ${selected === value ? 'selected' : ''}>${label}</option>`).join(''); }
