...
export type QtInstrumentProfileFamily = 'generic' | 'dmm' | 'power-supply' | 'signal-generator' | 'oscilloscope';
export type QtInstrumentProfileControlKind = 'measurement' | 'numeric' | 'toggle' | 'action';

export interface QtInstrumentProfileAction {
  id: string;
  label: string;
  kind: QtInstrumentProfileControlKind;
  query: string;
  write: string;
  unit: string;
  minimum: number;
...
export interface QtInstrumentProfileCapability {
  id: string;
  label: string;
  category: string;
  actionIds: string[];
...
export interface QtInstrumentProfileDocument {
  schemaVersion: number;
  id: string;
  displayName: string;
  family: QtInstrumentProfileFamily;
  manufacturer: string;
  model: string;
  documentationUrl: string;
  description: string;
  actions: QtInstrumentProfileAction[];
  capabilities: QtInstrumentProfileCapability[];
}

export interface QtInstrumentProfileCatalogEntry {
  catalogId: string;
  status: 'starter' | 'verified' | 'custom';
  notes: string;
  profile: QtInstrumentProfileDocument;
}

export interface QtInstrumentProfileValidationIssue {
  severity: 'warning' | 'error';
  message: string;
  actionId?: string;
}

export interface QtScpiIdentity {
  raw: string;
  manufacturer: string;
  model: string;
  serialNumber: string;
...
export interface QtInstrumentProfileMatchResult {
  entry: QtInstrumentProfileCatalogEntry;
  score: number;
...
export class QtInstrumentProfileEditorPanel implements vscode.Disposable {
  private readonly panel: vscode.WebviewPanel;
  private readonly disposables: vscode.Disposable[] = [];
  private document: QtInstrumentProfileDocument;
  private savedSnapshot: string;
  private profilePath: string | undefined;
  private statusMessage = '';

  constructor(
    initialProfilePath: string | undefined,
    private readonly projectRoot: string,
    private readonly catalog: QtInstrumentProfileCatalogEntry[],
    ...
    this.disposables.push(
      this.panel.onDidDispose(() => { this.onDisposed(); this.dispose(); }),
  ...
  private async handleMessage(raw: unknown): Promise<void> {
    ...
    const message = raw as {
      type?: string;
      document?: QtInstrumentProfileDocument;
      catalogId?: string;
      documentationUrl?: string;
    };
    if (message.document) this.document = normalizeQtInstrumentProfile(message.document);
    ...
    switch (message.type) {
      ...
      case 'openProfile':
        ...
        break;
      case 'openJson':
        if (this.profilePath) await vscode.window.showTextDocument(vscode.Uri.file(this.profilePath), { preview: false });
        else await this.save(true, true);
        break;
      case 'newProfile':
  ...
  private async save(forceSaveAs: boolean, openAfter = false): Promise<void> {
    const issues = validateQtInstrumentProfile(this.document);
    const errors = issues.filter((issue) => issue.severity === 'error');
    if (errors.length > 0) {
      const choice = await vscode.window.showWarningMessage(
        `The instrument profile contains ${errors.length} error(s). Save anyway?`,
        { modal: true },
        'Save anyway'
      );
    ...
    this.onSaved();
    this.render();
    if (openAfter) await vscode.window.showTextDocument(vscode.Uri.file(target), { preview: false });
  }
  ...
  private loadCatalogEntry(catalogId: string | undefined): void {
    const entry = this.catalog.find((candidate) => candidate.catalogId === catalogId);
    if (!entry) return;
    const cloned = normalizeQtInstrumentProfile(JSON.parse(JSON.stringify(entry.profile)) as QtInstrumentProfileDocument);
  ...
  private async openDocumentation(rawUrl: string | undefined): Promise<void> {
...
export function createBlankInstrumentProfile(): QtInstrumentProfileDocument {
  return {
    schemaVersion: 1,
...
export function normalizeQtInstrumentProfile(raw: QtInstrumentProfileDocument | Record<string, unknown>): QtInstrumentProfileDocument {
  const value = raw as Partial<QtInstrumentProfileDocument> & { actions?: unknown[]; capabilities?: unknown[] };
  const familyValues: QtInstrumentProfileFamily[] = ['generic', 'dmm', 'power-supply', 'signal-generator', 'oscilloscope'];
  const family = familyValues.includes(value.family as QtInstrumentProfileFamily) ? value.family as QtInstrumentProfileFamily : 'generic';
  return {
    schemaVersion: 1,
...
function normalizeAction(raw: unknown): QtInstrumentProfileAction {
  const value = (raw && typeof raw === 'object' ? raw : {}) as Record<string, unknown>;
  const kinds: QtInstrumentProfileControlKind[] = ['measurement', 'numeric', 'toggle', 'action'];
  const kind = kinds.includes(value.kind as QtInstrumentProfileControlKind) ? value.kind as QtInstrumentProfileControlKind : 'measurement';
...
function normalizeCapability(raw: unknown): QtInstrumentProfileCapability {
  const value = (raw && typeof raw === 'object' ? raw : {}) as Record<string, unknown>;
...
function inferCapabilitiesFromActions(actions: QtInstrumentProfileAction[], family: QtInstrumentProfileFamily): QtInstrumentProfileCapability[] {
  const map: Record<string, { id: string; label: string; category: string }> = {
  ...
  const result: QtInstrumentProfileCapability[] = [];
...
export function readQtInstrumentProfile(filePath: string): QtInstrumentProfileDocument {
  let parsed: unknown;
  try {
  ...
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) throw new Error('Instrument profile root must be a JSON object.');
  const normalized = normalizeQtInstrumentProfile(parsed as Record<string, unknown>);
...
export function serializeQtInstrumentProfile(profile: QtInstrumentProfileDocument): string {
  ...
  if (normalized.manufacturer) root.manufacturer = normalized.manufacturer;
  if (normalized.model) root.model = normalized.model;
  if (normalized.documentationUrl) root.documentationUrl = normalized.documentationUrl;
  if (normalized.description) root.description = normalized.description;
  root.actions = normalized.actions.map((action) => {
    ...
    if (action.query) result.query = action.query;
    if (action.write) result.write = action.write;
    if (action.unit) result.unit = action.unit;
...
export function validateQtInstrumentProfile(profile: QtInstrumentProfileDocument): QtInstrumentProfileValidationIssue[] {
  const issues: QtInstrumentProfileValidationIssue[] = [];
  if (!profile.id.trim()) issues.push({ severity: 'error', message: 'Profile ID is required.' });
  else if (!/^[A-Za-z0-9._-]+$/.test(profile.id)) issues.push({ severity: 'error', message: 'Profile ID may only contain letters, digits, dot, underscore and hyphen.' });
  if (!profile.displayName.trim()) issues.push({ severity: 'error', message: 'Display name is required.' });
  if ((profile.manufacturer && !profile.model) || (!profile.manufacturer && profile.model)) issues.push({ severity: 'warning', message: 'Manufacturer and model are normally specified together for model-specific profiles.' });
  if (profile.documentationUrl && !isPlausibleDocumentationUrl(profile.documentationUrl)) issues.push({ severity: 'warning', message: 'Documentation URL is neither an http(s) URL nor an obvious local/project-relative path.' });
  if (profile.actions.length === 0) issues.push({ severity: 'error', message: 'At least one SCPI action is required.' });
  const ids = new Set<string>();
  for (const action of profile.actions) {
    const prefix = action.id ? `Action "${action.id}"` : 'An action';
    if (!action.id) issues.push({ severity: 'error', message: 'Every action requires a non-empty ID.' });
    else if (!/^[A-Za-z0-9._-]+$/.test(action.id)) issues.push({ severity: 'error', message: `${prefix} has an invalid ID.`, actionId: action.id });
    else if (ids.has(action.id.toLowerCase())) issues.push({ severity: 'error', message: `Duplicate action ID: ${action.id}.`, actionId: action.id });
    else ids.add(action.id.toLowerCase());
    if (!action.label) issues.push({ severity: 'error', message: `${prefix} requires a label.`, actionId: action.id });
    if (action.kind === 'measurement' && !action.query.trim()) issues.push({ severity: 'error', message: `${prefix} is a measurement but has no query command.`, actionId: action.id });
    if (action.kind === 'action' && !action.write.trim()) issues.push({ severity: 'error', message: `${prefix} is an action but has no write command.`, actionId: action.id });
    if (action.kind === 'numeric') {
      if (!action.write.trim()) issues.push({ severity: 'error', message: `${prefix} is numeric but has no write template.`, actionId: action.id });
      else if (!action.write.includes('%1')) issues.push({ severity: 'warning', message: `${prefix} numeric write template does not contain %1, so the entered value will not be inserted.`, actionId: action.id });
      if (action.minimum > action.maximum) issues.push({ severity: 'error', message: `${prefix} minimum is greater than maximum.`, actionId: action.id });
      if (action.defaultValue < action.minimum || action.defaultValue > action.maximum) issues.push({ severity: 'warning', message: `${prefix} default value is outside its configured numeric range.`, actionId: action.id });
    }
    if (action.kind === 'toggle') {
      if (!action.write.trim()) issues.push({ severity: 'error', message: `${prefix} is a toggle but has no write template.`, actionId: action.id });
      else if (!action.write.includes('%1')) issues.push({ severity: 'warning', message: `${prefix} toggle write template does not contain %1; the control cannot insert ON/OFF state.`, actionId: action.id });
    }
    if (action.readback && !action.query.trim() && action.kind !== 'action') issues.push({ severity: 'warning', message: `${prefix} enables readback but does not define a query.`, actionId: action.id });
    if (action.decimals < 0 || action.decimals > 15) issues.push({ severity: 'warning', message: `${prefix} decimals should be between 0 and 15.`, actionId: action.id });
  }
  ...
  for (const capability of profile.capabilities) {
    ...
    if (!capability.id) issues.push({ severity: 'error', message: 'Every capability requires a non-empty ID.' });
    else if (!/^[A-Za-z0-9._-]+$/.test(capability.id)) issues.push({ severity: 'error', message: `${prefix} has an invalid ID.` });
    else if (capabilityIds.has(capability.id.toLowerCase())) issues.push({ severity: 'error', message: `Duplicate capability ID: ${capability.id}.` });
    else capabilityIds.add(capability.id.toLowerCase());
    if (!capability.label) issues.push({ severity: 'error', message: `${prefix} requires a label.` });
    if (!capability.category) issues.push({ severity: 'warning', message: `${prefix} has no category.` });
    if (capability.actionIds.length === 0) issues.push({ severity: 'warning', message: `${prefix} is not linked to any profile action.` });
    for (const actionId of capability.actionIds) {
      if (!actionIds.has(actionId.toLowerCase())) issues.push({ severity: 'error', message: `${prefix} references unknown action "${actionId}".` });
    }
  }
  return issues;
}

export function readQtInstrumentProfileCatalog(catalogPath: string): QtInstrumentProfileCatalogEntry[] {
  let raw: CatalogFile;
  try {
    raw = JSON.parse(fs.readFileSync(catalogPath, 'utf8')) as CatalogFile;
  } catch (error) {
    throw new Error(`Unable to read SCPI instrument profile catalog: ${error instanceof Error ? error.message : String(error)}`);
  }
  const entries: QtInstrumentProfileCatalogEntry[] = [];
  for (const candidate of raw.entries ?? []) {
    if (!candidate || typeof candidate !== 'object') continue;
    const value = candidate as Record<string, unknown>;
    if (!value.profile || typeof value.profile !== 'object') continue;
    const profile = normalizeQtInstrumentProfile(value.profile as Record<string, unknown>);
    if (!profile.id || !profile.displayName || profile.actions.length === 0) continue;
...
export function parseScpiIdentity(response: string): QtScpiIdentity {
...
export function matchScpiIdentityToCatalog(catalog: QtInstrumentProfileCatalogEntry[], response: string): QtInstrumentProfileMatchResult[] {
...
function scoreCatalogEntry(entry: QtInstrumentProfileCatalogEntry, identity: QtScpiIdentity): QtInstrumentProfileMatchResult {
  ...
  if (profileManufacturer === 'generic') return { entry, score: 0, confidence: 'none', reason: 'Generic profiles are not auto-detection candidates.' };
  if (profileManufacturer && profileManufacturer === identityManufacturer) { score += 30; reasons.push('manufacturer match'); }
  if (profileModel && identityModel) {
    if (profileModel === identityModel) { score += 70; reasons.push('exact model match'); }
    else if (identityModel.includes(profileModel) || profileModel.includes(identityModel)) { score += 55; reasons.push('model token match'); }
  }
  score = Math.max(0, Math.min(100, score));
  const confidence: QtInstrumentProfileMatchResult['confidence'] = score >= 90 ? 'high' : score >= 60 ? 'medium' : score > 0 ? 'low' : 'none';
  return { entry, score, confidence, reason: reasons.join(' + ') || 'No manufacturer/model match.' };
}

function compactScpiIdentityToken(value: string): string {
  return String(value ?? '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '');
}

function canonicalScpiManufacturer(value: string): string {
  const compact = compactScpiIdentityToken(value);
  if (compact.includes('keysight') || compact.includes('agilent') || compact.includes('hewlettpackard')) return 'keysight';
  if (compact.includes('rigol')) return 'rigol';
  if (compact.includes('fluke')) return 'fluke';
  if (compact.includes('tektronix')) return 'tektronix';
  if (compact.includes('rohdeschwarz') || compact === 'rs') return 'rohdeschwarz';
  if (compact.includes('siglent')) return 'siglent';
  return compact;
}
...
function isPlausibleDocumentationUrl(value: string): boolean {
